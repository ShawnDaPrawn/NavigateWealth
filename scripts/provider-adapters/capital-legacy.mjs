...
async function findCapitalLegacyDocumentAction(page, options, runtime) {
  ...
  if (artifactId === 'policy_schedule' || documentType === 'policy_schedule') {
    ...
    return findCapitalLegacyLabelAction(
      page,
      ['View Current Plan Schedule', 'Current Plan Schedule'],
      runtime,
    );
  }
  ...
  if (
    ...
    return findCapitalLegacyLabelAction(
      page,
      ['View Last Signed Will', 'Last Signed Will'],
      runtime,
    );
  }
  ...
  return evaluateWithNavigationRetry(
    ...
    (policyNumber) => {
      ...
      const valueAfter = (labelRegex, options = {}) => {
        ...
        if (textMatch && typeof textMatch.index === 'number') {
          const segment = bodyText.slice(
            textMatch.index,
            Math.min(bodyText.length, textMatch.index + 220),
          );
          if (matcher) {
      ...
      return {
        ...
        nextAnniversaryDate: valueAfter(/\bNext\s+anniversary\s+date\b/i, {
          valueRegex: isoDateRegex,
        }),
        diagnostics: {
...
async function readCapitalLegacyWillInfo(page, runtime) {
  ...
  return evaluateWithNavigationRetry(page, () => {
    ...
    const hasSignedWill =
      buttonVisible || /^yes$/i.test(signedCopy) || /^yes$/i.test(signedOriginal);
...
async function findCapitalLegacyLabelAction(page, labels, runtime) {
  ...
  const { evaluateWithNavigationRetry } = runtime;
  const selector = await evaluateWithNavigationRetry(
    page,
    (labelList) => {
      const normalise = (value) =>
        String(value || '')
      ...
      const labelRegexes = (labelList || []).map(
        (label) => new RegExp(String(label).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i'),
      );
    ...
    (targetSection) => {
      ...
      const clickables = Array.from(
        document.querySelectorAll(
          'a, button, [role="tab"], [role="link"], [role="button"], [role="menuitem"]',
        ),
      )
        .filter(isVisible)
        .map((el) => {
