export const allanGrayAdapter = {
  ...
  buildMissingMappedCurrentValueError(details) {
    const { fallbackCurrentValue, providerFallback, configuredFields, pageUrl } = details || {};
  ...
  const inferredSelector = await evaluateWithNavigationRetry(page, () => {
    ...
    const clickables = Array.from(
      document.querySelectorAll(
        'a, button, [role="link"], [role="button"], [role="menuitem"], [tabindex], [onclick], .mat-icon-button, .mat-mdc-icon-button',
      ),
    )
      .filter(isVisible)
      .map((el) => {
        ...
        const toolbarPosition =
          rect.top >= 90 && rect.top <= 285 && rect.left >= window.innerWidth * 0.68;
        const accountToolbarDownloadZone =
          rect.top >= 145 &&
          rect.top <= 245 &&
        ...
        const iconLike =
          Boolean(
            el.querySelector('svg, mat-icon, i, .material-icons, .mat-icon, [class*="icon" i]'),
          ) || /download|file_download|cloud_download|picture_as_pdf|pdf/i.test(text);
        const excluded =
          /zar|last\s+1\s+year|agra\d|account|client|user|profile|logout|south africa/i.test(text);
        return {
          el,
          text,
          rect,
          score:
            (accountToolbarDownloadZone ? 160 : 0) +
    ...
    (policyNumber) => {
      ...
      const labelledValue = (labelRegex, valueRegex) => {
        ...
        const segment = bodyText.slice(
          labelMatch.index,
          Math.min(bodyText.length, labelMatch.index + 260),
        );
      ...
      const visibleEntries = Array.from(document.querySelectorAll('body *'))
      ...
      const nearestMoneyAfterLabel = (labelRegex) => {
        const labels = visibleEntries.filter(
          (entry) => labelRegex.test(entry.text) && !extractMoney(entry.text),
        );
        ...
        for (const label of labels) {
          for (const money of monies) {
            ...
            const justBelow =
              money.rect.top >= label.rect.top && money.rect.top - label.rect.bottom <= 90;
        ...
        return best
          ? addCandidate(best.value, `near ${best.label}`, best.distance, best.context)
          : '';
      };

      const labelledMoney = (label, labelRegex) => {
        const value =
          lineValueAfter(labelRegex, /R\s*[\d\s,]+(?:\.\d{1,2})?/i, label) ||
          labelledValue(labelRegex, /R\s*[\d\s,]+(?:\.\d{1,2})?/i);
        return value ? addCandidate(value, label, 0, label) : '';
      };

      result.currentValue =
        labelledMoney('Total value', /\bTotal\s+value\s*\??/i) ||
      ...
      result.dateOfInception =
        lineValueAfter(
          /\bInception\s+date\b/i,
          /\b\d{1,2}\s+[A-Za-z]{3,9}\s+\d{4}\b/i,
          'Inception date',
        ) ||
        lineValueAfter(
          /\bDate\s+of\s+inception\b/i,
          /\b\d{1,2}\s+[A-Za-z]{3,9}\s+\d{4}\b/i,
          'Date of inception',
        ) ||
      ...
      if (policyRow) {
        const productCell = policyRow.find(
          (cell) =>
        ...
        if (valueCell && !result.currentValue) {
          result.currentValue = addCandidate(
            extractMoney(valueCell),
            'policy row',
            20,
            policyRow.join(' | '),
          );
      ...
      if (!result.currentValue) {
        const allAmounts = Array.from(bodyText.matchAll(/R\s*[\d\s,]+(?:\.\d{1,2})?/gi)).map(
          (match) => match[0].trim(),
        );
        result.currentValue = addCandidate(
          allAmounts[0] || '',
          'first page amount fallback',
          100,
      ...
      result.diagnostics.labelledCandidates = visibleEntries
        .filter((entry) =>
          /\b(total\s+value|closing\s+balance|inception\s+date|retirement\s+annuity|value)\b/i.test(
            entry.text,
          ),
        )
        .slice(0, 30)
