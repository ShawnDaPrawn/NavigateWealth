  ...
  return evaluateWithNavigationRetry(
    ...
    (policyNumber) => {
      ...
      const moneyValues = (value) =>
        Array.from(normalise(value).matchAll(/R\s*\d[\d\s,]*(?:\.\d{1,2})?/gi)).map((match) =>
          match[0].trim(),
        );
      ...
      const coverSummaryRoot = Array.from(
        document.querySelectorAll('section, main, article, div, table'),
      ...
      const extractVisualGridMoney = (headerRegex, rowRegex, valueKind) => {
        ...
        const candidates = moneyEntries
          ...
          .map((entry) => ({
            ...entry,
            distance:
              Math.abs(entry.rect.centerX - header.rect.centerX) +
              (entry.rect.centerX < row.rect.right ? 600 : 0),
          }))
          .sort((a, b) => a.distance - b.distance);
        const best = candidates[0];
        if (!best || best.distance > 420) return '';
        result.diagnostics.textSnippets.push(
          `${header.text} / ${row.text}: ${best.text}`.slice(0, 240),
        );
      ...
      result.severeIllness =
        extractVisualGridMoney(
          /that\s+you\s+can\s+recover\s+from/i,
          /additional\s+expense\s+needs/i,
          'lump_sum',
        ) ||
        extractFromCoverSegment(/additional\s+expense\s+needs/i, /this\s+table\s+shows/i, 'first');
