...
--
-- ORDERING: apply this AFTER the edge function that stops embedding
-- `client:clients(*)` is deployed. That embed is resolved by PostgREST through
-- the old foreign key; dropping the key first would break GET /calendar/events
-- until the deploy lands. The service now derives `client` from `attendees`.
--
