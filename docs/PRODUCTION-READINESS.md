...
**Expect this immediately after saving.** Three PRs are open (#233, and drafts
#238 and #239) whose heads were all pushed before the promotion. None of them
has an `e2e-smoke` result on its head SHA, so each will park at "Expected —
waiting for status" and refuse to merge. Two ways out, both fine:
