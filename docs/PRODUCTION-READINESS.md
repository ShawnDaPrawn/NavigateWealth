...
- [ ] Backup, DR, POPIA, FAIS, error-monitoring, CSP, and environment-split
      ...
      - [ ] **CSP** — STAGE 1 OF 2 still, but the blockers named here in the
            ...
            **The earlier "8 public routes, zero violations" was the wrong
            ...
            fonts from `public/pdfjs/` (`scripts/copy-pdfjs-assets.mjs`), which
      ...
      - [x] **DR** — a real restore now runs every week, and the earlier
            framing of this item was wrong in a way worth recording.

            It said the bar was "restoring into a throwaway project", which made
            DR look like it was blocked on money. It was not. The thing that
            actually needed proving is whether the dump loads into a live schema
            with every row intact, and a throwaway Postgres proves that as well
            as a throwaway Supabase project does. `weekly-backup.yml` now brings
            up a `postgres:17` service container, creates the Supabase roles the
            dump's policies reference, restores into it, and diffs an exact
            per-table row count against counts taken from the source at dump
            time. A mismatch fails the job.

            The restore deliberately does NOT use `--exit-on-error`. A vanilla
            Postgres has no `pg_net`, `pgsodium`, `supabase_vault`, `pg_graphql`
            or `pg_cron`, so those `CREATE EXTENSION` statements must fail and
            that is not a backup defect. Errors are counted and printed; the
            gate is the row comparison, so a restore that loses data fails even
            with zero errors, and one that logs extension noise passes if every
            row arrived.

            Counts are exact `count(*)`, not `reltuples`. That is not
            pedantry — the estimate-based table listing reported `events` and
            `reminders` as 0 rows when they hold 5 and 1, and an estimate can
            never prove a restore is complete.

            RESIDUAL RISK, since this is not everything a second project would
            give: it does not exercise Supabase's own platform restore, it does
            not cover Storage objects (the dump holds only their metadata, so
            files deleted after a backup are not recoverable from it), and it
            does not cover auth users.
...
| ---------------------- | ---------------------------------------- | ------------------------------------------------------------------------ | --------------------------------------------------------------- |
| Staging project        | Not affordable                           | Vercel previews, two required CI checks, weekly schema restore           | Migrations reach production with no prior run                   |
...
decoded.` followed by `DR rehearsal passed: N tables restored, every row count
identical.`, and an artifact `db-backup-<run_id>` retained 90 days.
...
| `Restored data does not match the source`   | The real one. The dump is incomplete — do not trust it, and read the printed diff |
...
| \n         | 2026-08-27                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Reframed the rubric for a real budget constraint rather than leaving two boxes unchecked forever. Closed **DR** at zero cost: the earlier framing said the bar was "a throwaway project", which made it look blocked on money — it was not. What needed proving is that the dump loads into a live schema with every row intact, and a `postgres:17` service container proves that as well as a second Supabase project does. `weekly-backup.yml` now restores into one weekly and diffs exact per-table counts against the source (exact `count(*)`, because the estimate-based listing reported `events`/`reminders` as 0 when they hold 5 and 1). Also credited what the Backup item never did: **Pro already takes a daily backup with 7-day retention**, so the wording implying an unprotected database was inaccurate — the workflow's real value is being off-vendor. Staging and PITR are now recorded as spending DECISIONS with named compensating controls and stated residual risk (migrations reach production unrehearsed; RPO up to 24h) instead of silent gaps. Confirmed repo/production migrations are not drifted. Accepted the RSS-thumbnail breakage per the owner: dropped the remote investing.com fallback for a local placeholder — it would have been blocked in turn, and reassigning `src` to a failing URL invites an onError loop — and removed that origin from `img-src`, so the policy is tighter than before. | Agent | \n  |
