...
- **A7 — the 853 MB of raw Figma images** — **DONE 2026-08-30.**
  ...
  generated a sibling, so every import fell through to a camera-resolution
  original (the largest 8256x5504, 31 MB). The cache stays out of git and is one
  of the few directories Vercel persists, so the resize is paid once (~18 s cold,
  ~1 s warm) rather than per deploy. Its key is a hash of file _contents_, not
  mtime: git does not preserve timestamps, so an mtime key would miss on every
  asset on every fresh checkout and regenerate them all, defeating the point.
  The resolver then falls through to the optimized siblings already committed in
  `src/assets` (`.webp`, `.avif`, `.jpg`, `.jpeg`) before the original, so a cold
  or failed cache degrades to a ~300 KB JPEG rather than silently reinstating the
  31 MB PNGs. `src/__tests__/figma-asset-weight.test.ts` measures that cold path
  deliberately — it is the half no build step is watching — and checks its own
  candidate list against `vite.config.ts` so the mirror cannot drift.
  **Measured:** 811.5 MB of originals → 10.0 MB of WebP (−98.8%);
  `imageBytes` 906 MB → 55.6 MB; `totalDistBytes` 927 MB → 70.9 MB. 82 emitted
  images, all WebP, largest 408 KB — held there by an encode ladder that
  re-compresses anything over a hard 500 KB ceiling and exits non-zero if a
  source ever defeats it, so the gate is a property of the pipeline rather than
  of today's files. Transparency is checked the same way: `stats().isOpaque`
  distinguishes a logo with real alpha from one carrying a redundant fully-opaque
  channel, and losing the former fails the build.
  **Residual:** the 55.6 MB that remains is `public/`, copied verbatim — the
  `img/optimized` variants plus `brand-assets`. The `< 50 MB` target was written
  before that composition was known, and closing the gap means decisions about
  material `public/brand-assets/README.md` documents as deliberate
  download/reference content (the high-res PNG fallbacks, `approved-source/`,
  the `rollback-2026-03-24/` archive). That is an owner call, not cleanup.
  Three imported assets still have no committed sibling and so are oversized on
  the cold path only (`KNOWN_UNOPTIMIZED` in the test); the generator converts
  all three, so nothing oversized ships from a normal build.
  _Gate:_ largest emitted image < 500 KB ✅; no 28 MB PNG reachable from any
  route ✅; `imageBytes` floored at 55.6 MB so the regression cannot return.
