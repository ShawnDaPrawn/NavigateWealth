...
2. [x] Client-led intake UI launched in production (all-at-once rollout)
...
1. Revert the frontend release that launched the client intake hub and adviser queue
2. Redeploy frontend — clients fall back to the previous published-results-only experience
3. Intake API remains available; no data loss if Postgres is canonical
