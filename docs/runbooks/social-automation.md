...
## Related: the Assets tab

Finished images and videos per channel live in a separate place with its own
contract — see [`social-channel-assets.md`](social-channel-assets.md). That is
what the **Assets** tab shows; this pipeline's candidate posts moved to the
**Weekly pipeline** tab beside it.

## Posting by hand

**Social Media → Compose** posts straight into Buffer: pick the channels, write
the text, attach an image from the Assets tab, then publish now, add to
Buffer's queue, schedule a time, or save a Buffer draft. The media itself is
added under Assets — see [`social-channel-assets.md`](social-channel-assets.md).
