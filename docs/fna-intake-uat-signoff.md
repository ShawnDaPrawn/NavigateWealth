...
**Environment:** client-led FNA intake UI enabled, Edge Function deployed, Postgres migration applied, `FNA_INTAKE_READ_FROM=postgres` after backfill.
...
- [x] Published FNA status routes to `ClientFNAView` results through `ClientFNAHub` — covered by Vitest `fna-intake-labels.test.ts`
- [x] Hub, wizard, queue — API UAT with intake UI enabled locally
