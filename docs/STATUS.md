...
| Social automation — X channel in Buffer | Open until the X account is connected in Buffer (third channel slot). The scheduling routine skips X until then.                                                                                                                                                                                                                                                                                                                                                                                                   | `runbooks/social-automation.md`                                    |
...
| Newsletter intake — migration           | Open until `supabase/migrations/…_newsletter_intake.sql` is applied through `apply_migration` (table `newsletter_intake` + two `service_role` functions). Until then the SQL hand-over path does not exist; the cron tick logs one warning and carries on.                                                                                                                                                                                                                                                         | `runbooks/newsletter-intake.md`                                    |
| Newsletter intake — secrets             | Open until `NW_NEWSLETTER_INTAKE_TOKEN` is set in Edge Function secrets (HTTPS hand-over path) and, optionally, `NW_NEWSLETTER_REVIEW_TO`. The SQL path needs neither.                                                                                                                                                                                                                                                                                                                                             | `runbooks/newsletter-intake.md`                                    |
| Newsletter intake — Routine             | Open until a monthly Routine (connector Supabase) exists that produces the PDF and runs the `newsletter-monthly-intake` skill. Until then admins upload the PDF by hand, which works today.                                                                                                                                                                                                                                                                                                                        | `runbooks/newsletter-intake.md`                                    |
...
| Point-in-Time Recovery | Declined at ~$100/mo for 7-day retention | Supabase daily backup (7-day), plus a weekly off-vendor dump that is restore-verified and a weekly Storage archive, both copied into an Object Lock bucket an account takeover cannot delete | RPO up to 24 hours: a failure at 23:00 can lose that day's work. The compensating controls raise the disaster floor; they do not narrow the window |
