...
| Calendar `client_id` foreign keys                 | **Open until applied.** `20260905170000_calendar_client_fk_to_auth_users.sql` re-points `events.client_id` and `reminders.client_id` from the empty `public.clients` table to `auth.users`. Apply it via the Supabase MCP `apply_migration` **after** the edge function that stops embedding `client:clients(*)` has deployed; until then, creating an event with a client attached fails with a foreign-key violation. | [`supabase/migrations/README.md`](../supabase/migrations/README.md)                                                         |
