...
how `quality/baselines/deno-check-baseline` was already run down to 0, and how F1 landed):
...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F1  | **Real module boundaries** — fix dependency-cruiser's resolver so first-party TS resolves, then the 3 boundary rules actually fire, ratcheted against a committed floor | §2.2, §2.4 module isolation    | **DONE + RATCHETED (Stage A, 2026-08-21)** — resolver fixed; 210 real violations surfaced as `warn` (109 cross-feature, 100 outsider-admin, 1 spa-edge type-only false positive) and gated by `quality/baselines/depcruise-baseline` in CI: the job now FAILS if the count rises above the floor. Burn the floor down, then flip the rules to `error` for a hard zero.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| F2  | **File-size budget ratchet** (1000 → 800 → 600, `error`), enforced via the total-warning floor in `quality/baselines/eslint-warning-baseline`                           | §2.2 module granularity        | **DONE (Stage A, 2026-08-21)** — warnings were wholly ungated, so `max-lines` warned constantly and blocked nothing. Removed 4 dead `eslint-disable` directives (59 → **55**) and ratcheted the TOTAL warning count, which is strictly stronger than a max-lines-only gate. Verified by probe: a new 1,100-line file pushes 55 → 56 and fails CI. Real `max-lines` count is **40** files (the rule skips blanks/comments and exempts `scripts/**`), not the 74 raw-line figure. Split those, then step the budget to 800 → 600, re-baselining each time; at 0 promote to `error`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| F3  | **Router-auth-guard**, extended to route-granularity via `route-auth-granular.test.ts` + `quality/baselines/route-auth-baseline`                                        | §3.4 auth coverage             | **DONE (Stage A, 2026-08-21)** — the module-granular test passes a module if ANY auth marker exists in its import tree, so a new unguarded route in an already-guarded file was invisible (`auth-routes.ts` passes with 10 unguarded routes). The new test counts **routes**: of **1032**, **127** have no visible guard; floor ratcheted. Resolves router-scoped `use('*')`, path-scoped `use('/links')`, parent inheritance via `parent.route()`, and in-handler checks. The count is a REVIEW LIST (it includes public-by-design routes like `/login`), not a vulnerability count — the value is the delta. **Baseline 173 → 127 on 2026-08-21** when main's PR #207 guarded 46 routes. That same PR exposed a design flaw in the original sanity check, which asserted the analysis could still re-derive the `documents.tsx` IDOR: #207 **fixed** that IDOR, so the check failed in CI for exactly the right reason. An integrity check must never be anchored on a defect staying unfixed. Re-anchored on properties stable under remediation — routes that are public _by definition_ (`/login-validate`, `/signup`, the health probes) must stay in the list, and `documents.tsx` must now stay OUT of it, which exercises both router- and path-scoped guard resolution.                                                                                                                              |
| F4  | **Per-layer coverage floors** — SPA via `vitest.config.ts`, backend via `quality/vitest.config.server.ts`, ratcheting                                                   | §8 test pyramid                | **DONE (Stage A, 2026-08-21)** — the ~136K-line backend was excluded from coverage entirely, so the blended "~31%" described the SPA only and the layer holding auth/PII/e-signatures was unmeasured. First honest backend measurement: **statements 13.43%, branches 9.38%, functions 12.88%, lines 13.79%** (573 tests). Floors set just below and gated in CI as a separate step, so the two layers now report as two numbers. **Ratcheted up 2026-08-21** to 13.6 / 9.7 / 13.3 / 14.0 after PR #207's security tests and the lazy-router suite brought the measurement to **13.74 / 9.80 / 13.48 / 14.11** across 608 tests.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| F10 | **One-way-to-do-it**: `react-toastify` banned at `error`; raw-`fetch` ratcheted via `quality/baselines/raw-fetch-baseline`                                              | §5 convergence                 | **DONE (Stage A, 2026-08-21)** — `react-toastify` banned at **`error`** after fixing the bug that made it bannable: its toasts were a **silent no-op** (no `<ToastContainer>` mounted anywhere, CSS never imported), so admins saving e-sign reminder settings got no success or failure feedback. Swapped to `sonner` (identical API, already used by every sibling), dependency removed, and dropped from the eager `vendor-feedback` chunk — entry graph 659.3 → **655.5 KB gzipped**. Raw `fetch` is **187** call sites (103 in just `publications/api.ts` and `social-media/api.ts`), far too many to error and too many to warn without drowning F2's 55-warning signal — so it is a **count ratchet** instead: the next call site cannot land silently. **Baseline 186 → 187 on 2026-08-21**: main's PR #207 added one call in `esignSignerService.ts`, a signer-facing file that deliberately uses the public anon key because signers are unauthenticated third parties. The shared `api` client attaches the _user's_ session token, so routing it there would be the wrong auth model — and that client's own anon-key fallback is a P1 finding, not a pattern to spread. Consistent with the other 9 calls in the same file. Banning direct `kv_store` imports is deliberately deferred — 177 files import it and the `repositories/` layer it should point at does not exist yet (§3.1, Stage B). |
...
_(Preconditions confirmed; **IMPLEMENTED 2026-08-21** — see the note at the end
of this item.)_
...
**What actually landed**, with three refinements the implementation forced:

1. **A sub-router that already set its own handler keeps it.** `honeycomb` is
   the one such router today. Hono's `onError` _replaces_ rather than chains, so
   attaching unconditionally would have silently discarded another module's
   explicit choice — the same class of bug this whole effort exists to remove.
   Detection identity-compares `router.errorHandler` against the value a fresh
   `new Hono()` carries. That field is `private` in Hono's published types even
   though Hono reads it as `app.errorHandler` in its own `mount()`
   implementation, so two tests pin the technique: a Hono upgrade that renames
   it fails CI instead of quietly disabling the net.
2. **`error.middleware.ts` is imported dynamically, not statically.**
   `lazy-router.ts` and `index.tsx` are both part of the deliberately minimal
   boot payload the lazy-mount architecture exists to protect. A static import
   would pull `error.middleware` plus `npm:zod`, the stderr logger and the
   quality-issues recorder into every cold start. Attachment also fails **open**
   — a missing safety net must never take down the route it protects.
3. **The request id has to cross the dispatch boundary.** The sub-router is a
   separate Hono instance with its own `Context`, so `index.tsx`'s
   `c.set('requestId')` is invisible inside it. Without forwarding, the shared
   handler would have recorded all 584 of those routes' failures with no
   correlation id — telemetry that cannot be joined to anything. `lazy-router`
   now forwards it as an `x-request-id` header (never overwriting one the caller
   supplied) and `errorHandler` falls back to it. This is a narrow, verifiable
   slice of B4 below; it does **not** solve request-scoped logging generally.

`index.tsx` also gets a root `onError` for what it genuinely does dispatch —
the health probes and any throw inside lazy-router's proxy handler — and stamps
`x-request-id` on the error response itself, because the existing middleware
sets that header only _after_ `await next()`, which never runs when the handler
throws.

**Typing trap, recorded because it cost a full measurement cycle.** The first
draft added `onError`/`errorHandler` to the `LazyRouterModule` structural type.
Hono declares `errorHandler` `private`, TypeScript compares private members
_nominally_, and every `Hono` instance immediately became structurally
incompatible: `deno check` went 34 → 111 errors, one per `lazy()` call site. The
error surface now lives in a separate type reached by an explicit cast at the
single place that needs it.
...
| `zValidator` on auth + esign (B2) | **MECHANISM DONE, ADOPTION RATCHETED.** `validate.ts` + 13 routes wired; `quality/baselines/route-validation-baseline` = 63 remaining.                                                                                                       |
...
| Consolidate auth onto `auth-mw`   | **PARTLY DONE + RATCHETED.** 7 → 5 hand-rolled implementations; `quality/baselines/auth-implementations-baseline` floors it. Found and fixed S12.                                                                                            |
| Introduce `repositories/`         | **SEEDED + RATCHETED.** Typed base with bounded reads by default; one namespace migrated; `quality/baselines/kv-direct-access-baseline` = 1780 direct `kv.*` calls (counts calls, not importing modules, so a file split cannot inflate it). |
...
incremental: `quality/baselines/contract-coverage-baseline` ratchets the number of validated
...
**`quality/baselines/contract-coverage-baseline` ratchets the other way round from every other
...
- All 40 currently exceed 800 and 600 as well, so stepping the budget does not
  ...
  `quality/baselines/eslint-warning-baseline` as part of the same change.
