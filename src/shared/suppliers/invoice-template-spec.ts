 ...
 *
 * Pure TypeScript with no imports, shared by the SPA and the edge function
 * (like shared/integrations/binding-utils.ts): the locked suppliers UI
 * re-exports it via suppliers/templateSpec.ts and the edge function via
 * server/locked/supplier-template-spec.ts, so the AI analyzer,
 * persistence-time normalization and the client renderer/editor can never
 * drift apart. Part of the Locked module — delete together with it (see
 * src/components/admin/modules/locked/README.md).
 */
