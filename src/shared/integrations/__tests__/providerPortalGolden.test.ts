...
describe('provider portal golden flows', () => {
  ...
  const providerFormSource = readRepoFile(
    'src/components/admin/modules/product-management/components/ProviderFormDialog.tsx',
  );
  const productProviderListSource = readRepoFile(
    'src/components/admin/modules/product-management/components/ProviderList.tsx',
  );
  ...
  const goldenDocs = readRepoFile('docs/architecture/provider-automation-golden-flows.md');
  ...
  it('preserves the Allan Gray default portal flow anchors', () => {
    ...
    expect(portalDefaultFlowsSource).toContain(
  ...
  it('preserves Allan Gray hooks behind the provider adapter boundary', () => {
    ...
    expect(allanGrayAdapterSource).toContain(
  ...
  it('keeps Capital Legacy extraction and download navigation behind its provider adapter boundary', () => {
    expect(adapterRegistrySource).toContain(
    ...
    expect(capitalLegacyAdapterSource).toContain(
  ...
  it('documents local watching and hosted replay for portal automation', () => {
    expect(readRepoFile('.github/workflows/provider-portal-worker.yml')).toContain(
    ...
    expect(readRepoFile('docs/architecture/provider-portal-worker.md')).toContain(
      'Watching automation',
    );
    expect(readRepoFile('docs/architecture/provider-portal-worker.md')).toContain(
      'Local live watching on this machine',
    );
    expect(readRepoFile('docs/architecture/provider-portal-worker.md')).toContain(
      'Hosted replay through GitHub Actions artifacts',
    );
    expect(readRepoFile('docs/architecture/provider-portal-worker.md')).toContain(
      'provider-portal-worker-<run id>',
    );
  ...
  it('provides a BrightRock risk portal flow without document download', () => {
    ...
    expect(portalDefaultFlowsSource).toContain(
      'BrightRock does not currently offer a direct cover-summary PDF download',
    );
  });
  ...
  it('keeps parent product categories out of portal automation', () => {
    ...
    expect(productProviderListSource).toContain(
      'getPortalAutomationCategoryOptions(provider.categoryIds)',
    );
    ...
    expect(integrationHeaderSource).toContain(
      'getPortalAutomationCategoryOptions(provider.categoryIds)',
    );
