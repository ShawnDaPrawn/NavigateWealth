...
function normalisePortalFieldMeaning(value: unknown): string {
  ...
  if (/(policy\s*(number|no)|account\s*number|investment\s*number|reference)/.test(normalised))
  ...
  if (
    /(product\s*type|product\s*name|investment\s*type|retirement\s*annuity\s*fund)/.test(normalised)
  )
    return 'product type';
  if (
    !/(maturity|estimated|projected|guaranteed|premium|contribution)/.test(normalised) &&
    /(current\s*value|fund\s*value|market\s*value|closing\s*balance|policy\s*value|account\s*value|retirement.*value|\bvalue\b)/.test(
      normalised,
    )
  )
    return 'current value';
  if (/(est\s*maturity\s*value|estimated\s*maturity\s*value)/.test(normalised))
    return 'est maturity value';

  return normalised;
}

export function buildIntegrationBindingsForFields<
  TField extends BindingFieldLike,
  TBinding extends IntegrationFieldBindingLike,
>(
  fields: TField[],
  bindings: TBinding[] = [],
  legacyFieldMapping: Record<string, string> = {},
): Array<
  IntegrationFieldBindingLike & {
    targetFieldId: string;
    targetFieldName: string;
    columnName: string;
    required: boolean;
...
> {
  ...
  for (const field of fields) {
    ...
    const legacyColumnName = Object.entries(legacyFieldMapping || {}).find(
      ([, targetFieldId]) => targetFieldId === field.id,
    )?.[0];
    const columnName = normaliseIntegrationColumnName(
      existing?.columnName || legacyColumnName || field.name || field.id,
    );
...
export function buildLegacyFieldMappingFromBindings(
  bindings: IntegrationFieldBindingLike[] = [],
  ...
  return Object.fromEntries(
    bindings
      .map(
        (binding) =>
          [
            normaliseIntegrationColumnName(binding.columnName),
            String(binding.targetFieldId || '').trim(),
          ] as const,
      )
...
export function mergeBindingIntoPortalField<
  TBinding extends IntegrationFieldBindingLike,
  TField extends PortalFlowFieldLike,
>(binding: TBinding, existing?: TField): PortalFlowFieldLike {
  const columnName = normaliseIntegrationColumnName(
    binding.columnName || existing?.columnName || existing?.sourceHeader,
  );
  const targetFieldId = String(binding.targetFieldId || existing?.targetFieldId || '').trim();
  const targetFieldName = String(
    binding.targetFieldName || existing?.targetFieldName || targetFieldId || columnName || 'Field',
  ).trim();
  ...
  const mergedLabels = Array.from(new Set([...bindingLabels, ...fallbackLabels])).slice(0, 12);
  ...
  return {
    ...
    targetFieldName,
    selector: String(binding.portalSelector || binding.selector || existing?.selector || '')
      .trim()
      .slice(0, 500),
    labels: mergedLabels.length > 0 ? mergedLabels : generatedLabels,
    attribute:
      String(existing?.attribute || 'text')
        .trim()
        .slice(0, 40) || 'text',
    required: binding.required === true || existing?.required === true,
    transform:
      String(binding.transform || existing?.transform || 'trim')
        .trim()
        .slice(0, 40) || 'trim',
  };
}

export function buildPortalFieldsFromBindings<
  TBinding extends IntegrationFieldBindingLike,
  TField extends PortalFlowFieldLike,
>(bindings: TBinding[] = [], existingFields: TField[] = []): PortalFlowFieldLike[] {
  if (!Array.isArray(bindings) || bindings.length === 0) {
    return existingFields.map((field) =>
      mergeBindingIntoPortalField(
        {
  ...
  const currentByColumn = new Map(
    existingFields
      .map(
        (field) =>
          [normaliseIntegrationColumnName(field.columnName || field.sourceHeader), field] as const,
      )
  ...
  const currentByMeaning = new Map(
    existingFields
      .map(
        (field) =>
          [
            normalisePortalFieldMeaning(
              field.targetFieldName || field.columnName || field.sourceHeader,
            ),
            field,
          ] as const,
      )
  ...
  return bindings
    .map((binding) => {
      ...
      const existing =
        currentByTarget.get(targetFieldId) ||
        currentByColumn.get(columnName) ||
        currentByMeaning.get(meaning);
