...
export const DEFAULT_TEMPLATES: Record<string, EmailTemplate> = {
  ...
  // --- Appointment booking lifecycle (Phase 1) -----------------------------
  // {{ .DetailsBlock }} and {{ .ActionsBlock }} are composed by
  // email-senders-appointments.ts: the details card varies between virtual
  // (join link) and in-person (location/directions) meetings, and the actions
  // row carries the Add-to-Google-Calendar / Reschedule / Cancel links.
  appointment_confirmation: {
    ...
    bodyHtml:
      '<p>Your appointment with {{ .AdvisorName }} has been booked. The details are below — an invite is attached so you can add it to your calendar.</p>{{ .DetailsBlock }}{{ .ActionsBlock }}',
    buttonLabel: '{{ .PrimaryLabel }}',
    buttonUrl: '{{ .PrimaryUrl }}',
    footerNote: 'Need to change this appointment? Use the reschedule link above.',
    category: 'appointments',
    isSystem: true,
  },
  appointment_reminder_day_before: {
    id: 'appointment_reminder_day_before',
    name: 'Appointment — Reminder (Day Before)',
    enabled: true,
    subject: 'Reminder: {{ .EventTitle }} tomorrow at {{ .EventTime }}',
    title: 'Appointment Tomorrow',
    subtitle: 'A friendly reminder about your appointment',
    greeting: 'Dear {{ .ClientName }},',
    bodyHtml:
      '<p>This is a reminder that your appointment with {{ .AdvisorName }} is <strong>tomorrow</strong>.</p>{{ .DetailsBlock }}{{ .ActionsBlock }}',
    buttonLabel: '{{ .PrimaryLabel }}',
    buttonUrl: '{{ .PrimaryUrl }}',
    footerNote: 'Need to change this appointment? Use the reschedule link above.',
    category: 'appointments',
  ...
  appointment_reminder_morning: {
    id: 'appointment_reminder_morning',
    name: 'Appointment — Reminder (Morning Of)',
    enabled: true,
    subject: 'Today: {{ .EventTitle }} at {{ .EventTime }}',
    title: 'Appointment Today',
    subtitle: 'Your appointment is today',
    greeting: 'Good morning {{ .ClientName }},',
    bodyHtml:
      '<p>Your appointment with {{ .AdvisorName }} is <strong>today at {{ .EventTime }}</strong>.</p>{{ .DetailsBlock }}{{ .ActionsBlock }}',
    buttonLabel: '{{ .PrimaryLabel }}',
    buttonUrl: '{{ .PrimaryUrl }}',
  ...
  appointment_starting_soon: {
    id: 'appointment_starting_soon',
    name: 'Appointment — Starting Soon',
    enabled: true,
    subject: 'Starting in 5 minutes: {{ .EventTitle }}',
    title: 'Starting Soon',
    subtitle: 'Your appointment is about to begin',
    greeting: 'Hi {{ .ClientName }},',
    bodyHtml:
      '<p>Your appointment <strong>{{ .EventTitle }}</strong> starts in about 5 minutes.</p>{{ .DetailsBlock }}',
    buttonLabel: '{{ .PrimaryLabel }}',
    buttonUrl: '{{ .PrimaryUrl }}',
    footerNote: '',
    category: 'appointments',
  ...
  appointment_rescheduled: {
    ...
    enabled: true,
    subject: 'Rescheduled: {{ .EventTitle }} — new time confirmed',
    title: 'Appointment Rescheduled',
    subtitle: 'Your appointment has a new time',
    greeting: 'Dear {{ .ClientName }},',
    bodyHtml:
      '<p>Your appointment with {{ .AdvisorName }} has been rescheduled. The updated details are below — the attached invite will update the entry in your calendar.</p>{{ .DetailsBlock }}{{ .ActionsBlock }}',
    buttonLabel: '{{ .PrimaryLabel }}',
    buttonUrl: '{{ .PrimaryUrl }}',
    footerNote: 'Need to change this appointment? Use the reschedule link above.',
    category: 'appointments',
  ...
  appointment_cancelled: {
    ...
    greeting: 'Dear {{ .ClientName }},',
    bodyHtml:
      '<p>Your appointment <strong>{{ .EventTitle }}</strong> on {{ .EventDate }} at {{ .EventTime }} has been cancelled. The attached invite will remove it from your calendar.</p><p>If you would like to book a new appointment, please contact us.</p>',
    buttonLabel: '',
    buttonUrl: '',
    footerNote: '',
    category: 'appointments',
  ...
  appointment_reschedule_requested: {
    id: 'appointment_reschedule_requested',
    name: 'Appointment — Reschedule Requested (Advisor)',
    enabled: true,
    subject: 'Reschedule requested: {{ .EventTitle }} — {{ .ClientName }}',
    title: 'Reschedule Requested',
    subtitle: 'A client has asked to move an appointment',
    greeting: 'Hello,',
    bodyHtml:
      '<p><strong>{{ .ClientName }}</strong> has requested to reschedule <strong>{{ .EventTitle }}</strong> (currently {{ .EventDate }} at {{ .EventTime }}).</p>{{ .DetailsBlock }}<p>Open the calendar to rebook — the client will automatically receive a new confirmation.</p>',
