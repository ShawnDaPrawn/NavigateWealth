...
app.post(
  ...
  asyncHandler(async (c) => {
    ...
    let bodyContent =
      emailBody || "<p>We'd like to invite you to complete a form on our website.</p>";
    ...
    const html = createEmailTemplate(bodyContent, {
      ...
      footerNote:
        'This invitation was sent on behalf of Navigate Wealth (FSP 54606). If you did not expect this invitation, you may safely ignore this email.',
      footerSettings,
    });
    ...
    if (!sent) {
      ...
      return c.json(
        {
          success: false,
          error: 'Email could not be sent. Please check your email configuration.',
        },
        500,
      );
    }
...
app.get(
  ...
  asyncHandler(async (c) => {
    ...
    if (hasFilters) {
      ...
      if (!parsed.success) {
        return c.json(
          { success: false, error: 'Invalid filter parameters', ...formatZodError(parsed.error) },
          400,
        );
      }
      filters = parsed.data;
...
app.post(
  ...
  asyncHandler(async (c) => {
    ...
    if (blockedDomain) {
      log.warn('Blocked public submission from scam domain', {
        type,
        submitterEmail,
        blockedDomain,
      });
