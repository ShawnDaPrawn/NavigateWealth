...
app.post(
  ...
  asyncHandler(async (c) => {
    ...
    const rateLimit = await checkPublicFormRateLimit('contact', email, (headerName) =>
      c.req.header(headerName),
    );
    if (!rateLimit.allowed) {
      log.info('Contact form rate limit exceeded', { limitedBy: rateLimit.limitedBy });
      return c.json(
        {
    ...
    const adminEmailOk = emailResults[0].status === 'fulfilled' && emailResults[0].value === true;
    const clientEmailOk = emailResults[1].status === 'fulfilled' && emailResults[1].value === true;
