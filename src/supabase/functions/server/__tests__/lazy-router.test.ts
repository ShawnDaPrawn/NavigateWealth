...
describe('deploy import graph has no dangling local specifiers', () => {
   ...
   * comment boundaries. A JSDoc example `import('./x.ts')` on this file was
   * enough to upload a missing asset and fail the Edge Function deploy with
   * HTTP 500. This walks the same server tree the CLI uploads (excluding
   * tests) and fails if any relative import() target does not exist.
   */
