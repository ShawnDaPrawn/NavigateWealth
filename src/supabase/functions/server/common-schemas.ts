 ...
 */
export const PastDateSchema = z.string().datetime().refine(pastDateRefinement, {
 ...
 */
export const FutureDateSchema = z.string().datetime().refine(futureDateRefinement, {
...
export const SaPhoneSchema = z
  ...
  .refine((val) => SA_PHONE_REGEX.test(val), {
...
export const OptionalSaPhoneSchema = z
  .string()
  .transform(sanitizePhone)
  .refine((val) => !val || SA_PHONE_REGEX.test(val), {
...
export const InternationalPhoneSchema = z
  .string()
  .transform(sanitizePhone)
  .refine((val) => INTERNATIONAL_PHONE_REGEX.test(val), {
 ...
 * Optional URL
 */
export const OptionalUrlSchema = z.string().regex(URL_REGEX, 'Invalid URL format').optional();
 ...
 */
export const DecimalCurrencySchema = z
  .number()
  .nonnegative('Amount cannot be negative')
 ...
 */
export const DecimalCommissionSplitSchema = z
  .number()
  .min(0, 'Commission split cannot be negative')
 ...
 */
export const CommunicationChannelSchema = z.enum(['email', 'sms', 'whatsapp', 'call', 'in_person']);
