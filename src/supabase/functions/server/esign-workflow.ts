...
export async function completeEnvelope(
  ...
  try {
    ...
    // 4. Merge Certificate
    const finalPdfBuffer = await PDFService.mergeCertificate(burnedPdfBuffer, certResult.pdfBuffer);
    ...
    try {
      ...
      if (fresh?.packet_run_id != null && typeof fresh.packet_step_index === 'number') {
        await advancePacketRunFromCompletion(
          envelopeId,
          fresh.packet_run_id,
          fresh.packet_step_index,
        );
      }
    } catch (packetErr) {
