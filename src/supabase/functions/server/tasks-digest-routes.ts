...
function normaliseTask(raw: RawKvTask): KvTask {
  ...
  return {
    ...
    is_template:
      raw.is_template ?? ((raw as Record<string, unknown>).isTemplate as boolean) ?? false,
    assignee_initials:
      raw.assignee_initials ??
      ((raw as Record<string, unknown>).assigneeInitials as string) ??
      null,
    assignee_id: raw.assignee_id ?? ((raw as Record<string, unknown>).assigneeId as string) ?? null,
    created_by:
...
async function requireCronOrAdminAuth(
  c: {
...
app.post(
  ...
  asyncHandler(async (c) => {
    ...
    if (lastSent?.date === todayKey && !force) {
      log.info('Digest already sent today — skipping (use ?force=true to override)', {
        date: todayKey,
      });
...
app.get('/', (c) => c.json({ service: 'tasks-digest', status: 'active', version: '1.0.0' }));
