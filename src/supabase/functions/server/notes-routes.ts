...
app.post(
  ...
  asyncHandler(async (c) => {
    ...
    const writes: Promise<void>[] = [kv.set(noteKey(id), note)];
...
app.post(
  ...
  asyncHandler(async (c) => {
    ...
    if (existing.convertedToTaskId) {
      return c.json(
        { error: 'Note has already been converted to a task', taskId: existing.convertedToTaskId },
        400,
      );
    }
    ...
    await Promise.all([kv.set(`task:${taskId}`, task), kv.set(noteKey(id), updated)]);
