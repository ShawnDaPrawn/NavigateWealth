  ...
  asyncHandler(async (c) => {
    ...
    const enriched = await Promise.all(
      logos.map(async (l) => {
        ...
        const displayAsset =
          assets.find((asset) => asset.storagePath === l.storagePath) || assets[0];
        return {
          ...l,
          assets,
...
app.post(
  ...
  asyncHandler(async (c) => {
    ...
    if (!allowedVariants.includes(variant)) {
      return c.json(
        { error: `Invalid variant. Must be one of: ${allowedVariants.join(', ')}` },
        400,
      );
    }
    ...
    let assets = uploadCandidates.filter(Boolean).filter(
      (
        candidate,
    ...
    if (assets.length === 0 && legacyFile) {
      ...
      const inferredFormat =
        legacyFile.type === 'application/pdf'
          ? 'pdf'
    ...
    const displayAsset =
      assets.find((asset) => asset.format === 'png') ||
...
app.put(
  ...
  asyncHandler(async (c) => {
    const { voice, updatedBy } = (await c.req.json()) as {
      voice: { tone: string; terminology: string; notes: string };
      updatedBy: string;
    };
