  ...
  asyncHandler(async (c) => {
    ...
    if (isFullStage && productDetails && typeof productDetails === 'object') {
      ...
      if (productDetails.phase === 2 && productDetails.risk_needs) {
        ...
        if (prefs) {
          if (prefs.cover_type)
            pdfFields.push({ label: 'Cover Type', value: String(prefs.cover_type) });
          if (prefs.network)
        ...
        if (fin) {
          ...
          if (fin.tax_bracket)
            pdfFields.push({ label: 'Tax Bracket', value: String(fin.tax_bracket) });
        }
        ...
        if (funding) {
          ...
          if (productId === 'ra') {
            ...
            productId === 'provident_preservation' ||
            productId === 'pension_preservation'
          ) {
            ...
            if (
              funding.leaving_employer_fund !== null &&
              funding.leaving_employer_fund !== undefined
            ) {
              pdfFields.push({
        ...
        if (tl) {
          if (tl.current_age)
            pdfFields.push({ label: 'Current Age', value: `${tl.current_age} years` });
        ...
        if (finR) {
          ...
          if (finR.tax_bracket)
            pdfFields.push({ label: 'Tax Bracket', value: String(finR.tax_bracket) });
        }
    ...
    if (isFullStage && productDetails && typeof productDetails === 'object') {
      ...
      if (productDetails.phase === 2 && productDetails.risk_needs) {
        ...
        if (pd) {
          ...
          if (pd.occupation)
            pRows.push(
              `<p style="margin: 4px 0;"><strong>Occupation:</strong> ${pd.occupation}</p>`,
            );
          if (pd.income_gross_monthly)
            pRows.push(
        ...
        if (hd) {
          ...
          if (hd.has_conditions === false) {
            healthLine =
              '<p style="margin: 4px 0; color: #16a34a;">No chronic conditions declared</p>';
          } else if (hd.has_conditions === true) {
        ...
        if (mem) {
          ...
          if (spouseM && (spouseM.dob || spouseM.age)) {
            const spouseInfo = spouseM.dob ? String(spouseM.dob) : `Age ${spouseM.age}`;
            mRows.push(
              `<p style="margin: 4px 0;"><strong>Spouse / Partner:</strong> ${spouseInfo}</p>`,
            );
          }
          const children = mem.children as Array<Record<string, unknown>> | undefined;
          if (children && children.length > 0) {
            children.forEach((child, i) => {
              const childInfo = child.dob
                ...
                : child.age
                  ...
                  : '—';
              mRows.push(
                `<p style="margin: 4px 0;"><strong>Child ${i + 1}:</strong> ${childInfo}</p>`,
              );
            });
          }
          if (mRows.length) {
        ...
        if (prefs) {
          ...
          if (prefs.cover_type)
            pRows.push(
              `<p style="margin: 4px 0;"><strong>Cover Type:</strong> ${prefs.cover_type}</p>`,
            );
          if (prefs.network)
            pRows.push(
          ...
          if (prefs.province)
            pRows.push(
              `<p style="margin: 4px 0;"><strong>Province:</strong> ${prefs.province}</p>`,
            );
          if (pRows.length) {
            sections.push(
        ...
        if (fin) {
          ...
          if (fin.tax_bracket)
            fRows.push(
              `<p style="margin: 4px 0;"><strong>Tax Bracket:</strong> ${fin.tax_bracket}</p>`,
            );
          if (fRows.length) {
            sections.push(
        ...
        if (funding) {
          ...
          if (productId === 'ra') {
            ...
            if (funding.transfer_not_sure) {
              fRows.push(
                `<p style="margin: 4px 0;"><strong>Transfer Amount:</strong> Not sure</p>`,
              );
            } else if (funding.transfer_amount) {
              fRows.push(
        ...
        if (tl) {
          ...
          if (tl.fund_details)
            tRows.push(
              `<p style="margin: 4px 0;"><strong>Fund(s):</strong> ${tl.fund_details}</p>`,
            );
          if (tRows.length) {
            sections.push(
        ...
        if (finR) {
          ...
          if (finR.tax_bracket)
            fRows.push(
              `<p style="margin: 4px 0;"><strong>Tax Bracket:</strong> ${finR.tax_bracket}</p>`,
            );
          if (fRows.length) {
            sections.push(
              `<h4 style="margin: 12px 0 4px; color: #374151;">Financial Snapshot</h4>${fRows.join('')}`,
        ...
        if (bdgt) {
          ...
          if (bRows.length) {
            sections.push(
              `<h4 style="margin: 12px 0 4px; color: #374151;">Budget</h4>${bRows.join('')}`,
            );
          }
        }
