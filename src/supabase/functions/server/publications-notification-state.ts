...
/**
 * Delivery-failure classification now lives in
 * `email-delivery-classification.ts` — it is not specific to article
 * notifications, and the client-communication module needs the same
 * rejected-vs-failed judgement. Re-exported here so every existing importer
 * (and this module's own test suite) keeps its import path.
 */
export {
  classifyDeliveryFailure,
  isSenderConfigurationFailure,
  normalizeSendError,
} from './email-delivery-classification.ts';
