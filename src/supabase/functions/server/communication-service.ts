...
import {
  processScheduledCampaigns as processScheduledCampaignsOld,
  resolveMergeFields,
} from './communication-business-logic.ts';
import type {
  Message,
  MessageCreate,
  Group,
...
export class CommunicationService {
      ...
      try {
        ...
        if (data.sendEmail && data.recipientEmail) {
          try {
            ...
            log.error(
              'Failed to send email, but message was saved to portal',
              emailError as Error,
  ...
  async uploadFile(file: File): Promise<StoredAttachment> {
    ...
    try {
      ...
      log.warn('Bucket check failed', { error: e instanceof Error ? e.message : String(e) });
    }
