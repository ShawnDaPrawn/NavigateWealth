...
import { SignerSubmitBodySchema, SignerRejectBodySchema } from './esign-validation.ts';
...
app.post(
  ...
  validateBody(SignerSubmitBodySchema),
  async (c) => {
    try {
...
app.post(
  ...
  requireIdempotency(),
  rateLimit('SIGNER_SUBMIT'),
  validateBody(SignerRejectBodySchema),
  async (c) => {
    try {
      const body = await c.req.json();
        ...
        if (envelopeForNotify?.created_by_user_id) {
          ...
          if (senderEmail) {
            ...
            if (declineDecision.deliver) {
              ...
              log.info(
                `Decline suppressed for sender ${senderEmail} (mode=${declineDecision.mode})`,
              );
            }
        ...
        if (envelopeForHook) {
          void emitWebhookEvent({
            ...
            payload: {
              ...
              envelope: {
                id: envelopeForHook.id,
                title: envelopeForHook.title,
                status: 'declined',
              },
              reason: reason || null,
            },
