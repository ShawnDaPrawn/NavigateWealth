      ...
      try {
        ...
        if (!response.ok) {
          if (data.error === 'Confirmation link expired') {
            ...
            setStatus('error');
            setMessage(
              data.error || 'Failed to confirm subscription. Please try again or contact support.',
            );
          }
          return;
        }
        ...
        if (data.alreadyConfirmed) {
          ...
          setStatus('success');
          setMessage(
            "Thank you for confirming your subscription! You'll now receive our newsletter with financial insights and updates.",
          );
        }
      } catch (error) {
