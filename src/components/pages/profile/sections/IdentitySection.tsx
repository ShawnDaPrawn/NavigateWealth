...
interface IdentitySectionProps {
  ...
  getDocumentTypeIcon: (type: IdentityDocumentType) => {
...
export function IdentitySection({
  ...
  return (
    <div className="contents">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                onClick={() => addIdentityDocument('national-id')}
                variant="outline"
                size="sm"
                disabled={hasDocumentType('national-id') || identityDocsInEditMode.size > 0}
                className="border-[#6d28d9] text-[#6d28d9] hover:bg-[#6d28d9]/10 disabled:opacity-50 disabled:cursor-not-allowed"
                title={
                  identityDocsInEditMode.size > 0
                    ? 'Please save the current document before adding a new one'
                    : hasDocumentType('national-id')
                      ? 'National ID already added'
                      : 'Add National ID'
                }
              >
                <Plus className="h-4 w-4 mr-1" />
                {hasDocumentType('national-id') ? 'ID Added' : 'Add National ID'}
              </Button>
              <Button
                onClick={() => addIdentityDocument('passport')}
                variant="outline"
                size="sm"
                disabled={hasDocumentType('passport') || identityDocsInEditMode.size > 0}
                className="border-blue-600 text-blue-600 hover:bg-blue-50 disabled:opacity-50 disabled:cursor-not-allowed"
                title={
                  identityDocsInEditMode.size > 0
                    ? 'Please save the current document before adding a new one'
                    : hasDocumentType('passport')
                      ? 'Passport already added'
                      : 'Add Passport'
                }
              >
                <Plus className="h-4 w-4 mr-1" />
                {hasDocumentType('passport') ? 'Passport Added' : 'Add Passport'}
              </Button>
              <Button
                onClick={() => addIdentityDocument('drivers-license')}
                variant="outline"
                size="sm"
                disabled={hasDocumentType('drivers-license') || identityDocsInEditMode.size > 0}
                className="border-amber-600 text-amber-600 hover:bg-amber-50 disabled:opacity-50 disabled:cursor-not-allowed"
                title={
                  identityDocsInEditMode.size > 0
                    ? 'Please save the current document before adding a new one'
                    : hasDocumentType('drivers-license')
                      ? "Driver's License already added"
                      : "Add Driver's License"
                }
              >
                <Plus className="h-4 w-4 mr-1" />
                {hasDocumentType('drivers-license') ? 'License Added' : 'Add License'}
              </Button>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          <Alert className="border-blue-200 bg-blue-50">
            ...
            <AlertDescription className="text-sm text-blue-800">
              Please upload clear copies of your identity documents. Accepted formats: PDF, JPG, PNG
              (Max 5MB). You can add <strong>one document of each type</strong> (National ID,
              Passport, or Driver's License). Click "Add" buttons above to add documents, then
              "Save" to confirm each entry.
            </AlertDescription>
          </Alert>
            ...
            <div className="space-y-4">
              {profileData.identityDocuments.map((doc) => {
                ...
                return (
                  <div
                    key={doc.id}
                    className={`p-5 rounded-lg border-2 ${isInEditMode ? `${colorClasses.border} bg-white` : 'border-gray-200 bg-white'}`}
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {doc.number && !isInEditMode && (
                            <p className="text-xs text-gray-600">ID: {doc.number}</p>
                          )}
                        </div>
                      </div>
                        ...
                        {!isInEditMode ? (
                          <div className="contents">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => editIdentityDocument(doc.id)}
                              className={`${colorClasses.border} ${colorClasses.text} ${colorClasses.hover}`}
                            >
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => confirmDeleteIdentityDocument(doc.id)}
                          ...
                          <div className="contents">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => cancelEditIdentityDocument(doc.id)}
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => saveIdentityDocument(doc.id)}
                              disabled={!isValid}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            ...
                            <Input
                              value={doc.number}
                              onChange={(e) =>
                                updateIdentityDocument(doc.id, { number: e.target.value })
                              }
                              placeholder={`Enter ${doc.type === 'national-id' ? 'ID' : doc.type === 'passport' ? 'passport' : 'license'} number`}
                              className="mt-1.5"
                            />
                          </div>
                          <div>
                            ...
                            <CountrySelect
                              ...
                              onValueChange={(value) =>
                                updateIdentityDocument(doc.id, { countryOfIssue: value })
                              }
                              className="mt-1.5"
                            />
                          </div>
                          {doc.type === 'passport' && (
                            <div>
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  updateIdentityDocument(doc.id, { expiryDate: e.target.value })
                                }
                                className="mt-1.5"
                              />
                            </div>
                        ...
                        <div>
                          ...
                          <div className="mt-1.5">
                            {doc.fileName ? (
                              <div className="flex items-center gap-3 p-4 rounded-lg border-2 border-green-200 bg-green-50">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <p className="text-xs text-gray-600">
                                    {((doc.fileSize || 0) / 1024).toFixed(0)} KB • Uploaded{' '}
                              ...
                              <label
                                className={`flex flex-col items-center gap-2 p-6 rounded-lg border-2 border-dashed border-gray-300 hover:${colorClasses.border} bg-gray-50 ${colorClasses.hover} cursor-pointer transition-all`}
                              >
                                <Upload className="h-8 w-8 text-gray-400" />
                                <div className="text-center">
                                  <p className="text-sm text-gray-700">
                                    Click to upload {getDocumentTypeLabel(doc.type).toLowerCase()}
                                  </p>
      ...
      <AlertDialog
        open={identityDocToDelete !== null}
        onOpenChange={() => setIdentityDocToDelete(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Identity Document</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete this identity document? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => identityDocToDelete && removeIdentityDocument(identityDocToDelete)}
