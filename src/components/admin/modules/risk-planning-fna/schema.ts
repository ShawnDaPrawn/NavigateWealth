...
export const InformationGatheringSchema = z
  .object({
    // Income Information
    ...
    retirementAge: z
      ...
      .refine(
        (val) =>
          Number(val) >= VALIDATION_RULES.MIN_RETIREMENT_AGE &&
          Number(val) <= VALIDATION_RULES.MAX_RETIREMENT_AGE,
    ...
    // Household
    totalHouseholdMonthlyExpenditure: z
      .string()
      .transform((val) => (val === '' ? '0' : val)) // Default empty to '0'
      .refine((val) => !isNaN(Number(val)), 'Must be a valid number')
      .refine((val) => Number(val) >= 0, 'Cannot be negative'),
  ...
  .refine((data) => Number(data.retirementAge) > Number(data.currentAge), {
    message: 'Retirement age must be greater than current age',
    path: ['retirementAge'],
  })
  .refine((data) => Number(data.netMonthlyIncome) <= Number(data.grossMonthlyIncome), {
...
export function transformFormToInput(formValues: InformationGatheringFormValues) {
  ...
  const clientIncomePercentage =
    combinedHouseholdIncome > 0 ? (grossMonthlyIncome / combinedHouseholdIncome) * 100 : 100;
