...
export const RiskPlanningFNAPdfExport = ({
  calculations,
  adjustments,
  clientName = 'Client',
  clientId,
}: RiskPlanningFNAPdfExportProps) => {
  if (!calculations) {
    return null;
  }
    ...
    <div key="page-1">
      ...
      <section className="section">
        ...
        <div className="callout mb-4">
          <p className="m-0">
            This Financial Needs Analysis (FNA) has been prepared for <strong>{clientName}</strong>{' '}
            as at {formatDate(calculations.metadata.calculatedAt)}. The analysis is based on the
            client's current financial position and circumstances as disclosed during the advisory
            process.
          </p>
        </div>
        ...
        <div className="mt-6">
          ...
          <table className="w-full">
            ...
            <tbody>
              <tr>
                <td>
                  <strong>Life Cover (Death)</strong>
                </td>
                <td className="text-right font-bold">
                  {formatCurrency(lifeCover?.finalRecommendedCover || 0)}
                </td>
              </tr>
              <tr>
                <td>
                  <strong>Lump Sum Disability Cover</strong>
                </td>
                <td className="text-right font-bold">
                  {formatCurrency(disabilityCover?.finalRecommendedCover || 0)}
                </td>
              </tr>
              <tr>
                <td>
                  <strong>Severe Illness Cover</strong>
                </td>
                <td className="text-right font-bold">
                  {formatCurrency(severeIllnessCover?.finalRecommendedCover || 0)}
                </td>
              </tr>
              <tr>
                <td>
                  <strong>Income Protection (Temporary)</strong>
                </td>
                <td className="text-right font-bold">
                  {formatCurrency(ipTemporary?.finalRecommendedCover || 0)}/month
                </td>
              </tr>
              <tr>
                <td>
                  <strong>Income Protection (Permanent)</strong>
                </td>
                <td className="text-right font-bold">
              ...
              <tr className="bg-gray-100">
                <td>
                  <strong>Total Lump Sum Cover</strong>
                </td>
                <td className="text-right font-bold text-purple-700">
                  {formatCurrency(totalLumpSumCover)}
                </td>
              </tr>
              <tr className="bg-gray-100">
                <td>
                  <strong>Total Monthly Income Protection (Permanent)</strong>
                </td>
                <td className="text-right font-bold text-purple-700">
                  {formatCurrency(permanentMonthlyIncome)}/month
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <section className="section">
        ...
        {lifeCover && lifeCover.assumptions.length > 0 && (
          <div className="mb-4">
            <h3 className="text-[9.5px] font-bold mb-1 uppercase text-gray-700">
              Life Cover (Death)
            </h3>
            <ul
        ...
        {(ipTemporary || ipPermanent) &&
          ipTemporary?.assumptions &&
          ipTemporary.assumptions.length > 0 && (
            <div className="mb-4">
              <h3 className="text-[9.5px] font-bold mb-1 uppercase text-gray-700">
                Income Protection
              </h3>
          ...
          <div className="contents">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td className="text-right">
                    {formatCurrency(lifeCover.existingCoverPersonal || 0)}
                  </td>
                </tr>
                <tr>
                  <th>Existing Cover (Group)</th>
                  <td className="text-right">
                    {formatCurrency(lifeCover.existingCoverGroup || 0)}
                  </td>
                </tr>
                <tr>
                  <th>
                    <strong>Total Existing Cover</strong>
                  </th>
                  <td className="text-right font-bold">
                    {formatCurrency(lifeCover.existingCoverTotal)}
                  </td>
                </tr>
                {lifeCover.isOverinsured ? (
                  <tr className="bg-amber-50">
                    ...
                    <td className="text-right font-bold text-amber-700">
                      {formatCurrency(lifeCover.overinsuredAmount)}
                    </td>
                  </tr>
                ) : (
                  <tr>
                ...
                <tr className="bg-gray-100">
                  <th>
                    <strong>Final Recommended Cover</strong>
                  </th>
                  <td className="text-right font-bold text-purple-700">
                    {formatCurrency(lifeCover.finalRecommendedCover)}
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            {lifeCover.isOverinsured && (
              <div className="callout mb-4 bg-amber-50 border-amber-200">
                ...
                <p className="text-[8.5px] m-0 text-amber-800">
                  The client's existing cover ({formatCurrency(lifeCover.existingCoverTotal)})
                  exceeds their calculated need ({formatCurrency(lifeCover.grossNeed)}) by{' '}
                  {formatCurrency(lifeCover.overinsuredAmount)}. Consider reducing cover to avoid
                  unnecessary premium expenditure.
                </p>
              </div>
            )}
            ...
            {lifeCover.advisorOverride && (
              <div className="callout mb-4 bg-[#eef2ff] border-[#e0e7ff]">
                ...
                <p className="text-[8.5px] m-0 mb-1">
                  <strong>Reason:</strong> {lifeCover.advisorOverride.reason}
                </p>
                <p className="text-[8.5px] m-0 mb-1">
                  <strong>Classification:</strong> {lifeCover.advisorOverride.classification}
                </p>
                <p className="text-[8.5px] m-0">
                  <strong>Overridden by:</strong> {lifeCover.advisorOverride.overriddenBy} on{' '}
                  {formatDate(lifeCover.advisorOverride.overriddenAt)}
                </p>
              </div>
            )}
            ...
            <div className="mt-4">
              ...
              >
                {lifeCover.riskNotes &&
                  lifeCover.riskNotes.map((note, idx) => (
                    <li key={idx} style={{ marginBottom: '2px' }}>
                      {note}
                    </li>
                  ))}
          ...
          <div className="contents">
            ...
            <table className="w-full mb-4">
              <tbody>
                ...
                <tr>
                  <th>Existing Cover (Personal)</th>
                  <td className="text-right">
                    {formatCurrency(disabilityCover.existingCoverPersonal || 0)}
                  </td>
                </tr>
                <tr>
                  <th>Existing Cover (Group)</th>
                  <td className="text-right">
                    {formatCurrency(disabilityCover.existingCoverGroup || 0)}
                  </td>
                </tr>
                <tr>
                  <th>
                  ...
                  <td className="text-right font-bold">
                    {formatCurrency(disabilityCover.existingCoverTotal)}
                  </td>
                </tr>
                {disabilityCover.isOverinsured ? (
                  <tr className="bg-amber-50">
                    <th>Overinsured Amount</th>
                    <td className="text-right font-bold text-amber-700">
                      {formatCurrency(disabilityCover.overinsuredAmount)}
                    </td>
                  </tr>
                ) : (
                  <tr>
                ...
                <tr className="bg-gray-100">
                  ...
                  <td className="text-right font-bold text-purple-700">
                    {formatCurrency(disabilityCover.finalRecommendedCover)}
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            {disabilityCover.isOverinsured && (
              <div className="callout mb-4 bg-amber-50 border-amber-200">
                <p className="text-[9px] font-bold mb-1 text-amber-900">OVERINSURED</p>
                <p className="text-[8.5px] m-0 text-amber-800">
                  The client's existing cover ({formatCurrency(disabilityCover.existingCoverTotal)})
                  exceeds their calculated need ({formatCurrency(disabilityCover.grossNeed)}) by{' '}
                  {formatCurrency(disabilityCover.overinsuredAmount)}. Consider reducing cover to
            ...
            {disabilityCover.advisorOverride && (
              <div className="callout mb-4 bg-[#eef2ff] border-[#e0e7ff]">
                <p className="text-[9px] font-bold mb-1">ADVISER OVERRIDE APPLIED</p>
                <p className="text-[8.5px] m-0 mb-1">
                  <strong>Reason:</strong> {disabilityCover.advisorOverride.reason}
                </p>
                <p className="text-[8.5px] m-0 mb-1">
                  <strong>Classification:</strong> {disabilityCover.advisorOverride.classification}
                </p>
                <p className="text-[8.5px] m-0">
                  <strong>Overridden by:</strong> {disabilityCover.advisorOverride.overriddenBy} on{' '}
                  {formatDate(disabilityCover.advisorOverride.overriddenAt)}
                </p>
              </div>
            )}
            ...
            <div className="mt-4">
              ...
              <ul
                ...
                style={{ margin: '0', paddingLeft: '15px' }}
              >
                {disabilityCover.riskNotes &&
                  disabilityCover.riskNotes.map((note, idx) => (
                    <li key={idx} style={{ marginBottom: '2px' }}>
                      {note}
                    </li>
                  ))}
          ...
          <table className="w-full mb-4">
            <tbody>
              ...
              <tr>
                ...
                <td className="text-right">
                  {formatCurrency(severeIllnessCover.existingCoverTotal)}
                </td>
              </tr>
              <tr>
                <th>Net Shortfall</th>
              ...
              <tr className="bg-gray-100">
                ...
                <td className="text-right font-bold text-purple-700">
                  {formatCurrency(severeIllnessCover.finalRecommendedCover)}
                </td>
              </tr>
            </tbody>
          </table>
              ...
              <table className="w-full">
                <tbody>
                  ...
                  <tr>
                    <th>Existing Cover (Total)</th>
                    <td className="text-right">
                      {formatCurrency(ipTemporary.existingCoverTotal)}/month
                    </td>
                  </tr>
                  <tr className="bg-gray-100">
                    <th>
                      <strong>Recommended Cover</strong>
                    </th>
                    <td className="text-right font-bold text-purple-700">
                      {formatCurrency(ipTemporary.finalRecommendedCover)}/month
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <table className="w-full">
                <tbody>
                  ...
                  <tr>
                    <th>Existing Cover (Total)</th>
                    <td className="text-right">
                      {formatCurrency(ipPermanent.existingCoverTotal)}/month
                    </td>
                  </tr>
                  <tr className="bg-gray-100">
                    <th>
                    ...
                    <td className="text-right font-bold text-purple-700">
                      {formatCurrency(ipPermanent.finalRecommendedCover)}/month
                    </td>
                  </tr>
                </tbody>
              </table>
