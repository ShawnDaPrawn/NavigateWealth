...
export function AutoContentPanel({ categories: _categories, onArticlesGenerated }: AutoContentPanelProps) {
  ...
  const triggerAllPipelines = useCallback(async () => {
    ...
    try {
      ...
      if (totalGenerated > 0) {
        toast.success(
          `${totalGenerated} draft article(s) generated across ${results.length} pipeline(s)`,
        );
        onArticlesGenerated?.();
      } else {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            AI-powered article generation from market data, regulatory changes, news, and calendar
            events. All generated articles land as drafts for your review.
          </p>
        </div>
                  ...
                  {triggerResult && triggerResult.pipelineId === config.id && (
                    ...
                    >
                      <div className="flex items-start gap-2">
                        <div
                          className={STATUS_CONFIG[triggerResult.status]?.color || 'text-gray-500'}
                        >
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-700">
                        {config.categoryName || 'Auto-detect'}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-wider text-gray-400 mb-1">
                      ...
                      <div className="flex items-center gap-2">
                        <Switch
                          ...
                          onCheckedChange={async (checked) => {
                            try {
                              await PublicationsAPI.AutoContent.updateConfig(config.id, {
                                autoPublish: checked,
                              });
                              setConfigs((prev) =>
                                prev.map((c) =>
                                  c.id === config.id ? { ...c, autoPublish: checked } : c,
                                ),
                              );
                              toast.success(
                                `Auto-publish ${checked ? 'enabled' : 'disabled'} for ${config.name}`,
                              );
                            } catch {
                        ...
                        <span
                          className={cn(
                            'text-xs font-medium',
                            config.autoPublish ? 'text-green-600' : 'text-gray-400',
                          )}
                        >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            await PublicationsAPI.AutoContent.updateConfig(config.id, {
                              scheduleIntervalHours: val,
                            });
                            setConfigs((prev) =>
                              prev.map((c) =>
                                c.id === config.id ? { ...c, scheduleIntervalHours: val } : c,
                              ),
                            );
                            toast.success(
                              val > 0
                  ...
                  <div>
                    ...
                    {isLoadingHistory ? (
                      ...
                      <p className="text-sm text-gray-400 py-3">
                        No runs yet. Click &quot;Run Now&quot; to generate articles.
                      </p>
                    ) : (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2">
                                <Badge variant="outline" className="text-[10px] h-5">
                                  {run.articlesGenerated} article
                                  {run.articlesGenerated !== 1 ? 's' : ''}
                                </Badge>
                                <span className="text-[10px] text-gray-400">
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-gray-500">
              Articles are automatically generated ahead of these events based on the configured
              lead time.
            </p>
          </CardHeader>
          <CardContent>
                      ...
                      <div className="text-right">
                        ...
                        {event.lastGeneratedYear && (
                          <p className="text-[10px] text-green-600">
                            Generated {event.lastGeneratedYear}
                          </p>
                        )}
                      </div>
                      <Badge
      ...
      <div className="bg-purple-50 border border-purple-100 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <ul className="text-xs text-purple-700 mt-1 space-y-0.5 list-disc list-inside">
            <li>
              Each pipeline fetches data from external sources (RSS feeds, calendar) and generates
              articles using AI
            </li>
            <li>
              All articles are created as <strong>drafts</strong> — nothing is ever auto-published
            </li>
            <li>
              Duplicate content is prevented via deduplication hashing — the same news item
              won&apos;t generate twice
            </li>
            <li>
              Use <strong>Content Sources</strong> above to add RSS feeds, control check frequency,
              and set daily/weekly generation caps
            </li>
            <li>
              Use &quot;Run Now&quot; to manually trigger any pipeline, or &quot;Run All&quot; for
              all enabled pipelines
            </li>
            <li>
              Enabled pipelines run automatically based on their <strong>schedule interval</strong>{' '}
              — checked every 15 minutes while the admin panel is open
            </li>
          </ul>
        </div>
      </div>
