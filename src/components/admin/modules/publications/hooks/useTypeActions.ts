...
export function useTypeActions(options?: UseTypeActionsOptions): UseTypeActionsReturn {
  ...
  const handleCreate = useCallback(
    async (data: CreateContentTypeInput): Promise<ContentType | null> => {
  ...
  const handleUpdate = useCallback(
    async (id: string, data: Partial<CreateContentTypeInput>): Promise<ContentType | null> => {
      setIsProcessing(true);
  ...
  const handleToggleActive = useCallback(
    async (type: ContentType): Promise<ContentType | null> => {
      setIsProcessing(true);
      ...
      try {
        ...
        if (onSuccess) {
          const message = updated.is_active ? 'Type activated' : 'Type deactivated';
  ...
  const handleReorder = useCallback(
    async (types: ContentType[]): Promise<boolean> => {
      setIsProcessing(true);
