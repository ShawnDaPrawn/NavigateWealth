...
export function useCategoryActions(options?: UseCategoryActionsOptions): UseCategoryActionsReturn {
  ...
  const handleCreate = useCallback(
    async (data: CreateCategoryInput): Promise<Category | null> => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : ERROR_MESSAGES.categoryCreateFailed;
  ...
  const handleUpdate = useCallback(
    async (id: string, data: Partial<CreateCategoryInput>): Promise<Category | null> => {
      setIsProcessing(true);
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : ERROR_MESSAGES.categoryUpdateFailed;
  ...
  const handleDelete = useCallback(
    async (id: string): Promise<boolean> => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : ERROR_MESSAGES.categoryDeleteFailed;
  ...
  const handleToggleActive = useCallback(
    async (category: Category): Promise<Category | null> => {
      setIsProcessing(true);
      ...
      try {
        ...
        if (onSuccess) {
          const message = updated.is_active ? 'Category activated' : 'Category deactivated';
  ...
  const handleReorder = useCallback(
    async (categories: Category[]): Promise<boolean> => {
      setIsProcessing(true);
