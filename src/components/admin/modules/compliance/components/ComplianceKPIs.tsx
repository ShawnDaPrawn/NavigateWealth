...
import { Shield, AlertTriangle, FileText, Users, Calendar } from 'lucide-react';
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            <Badge {...getStatusBadge(kpis.fspLicence.status)}>{kpis.fspLicence.status}</Badge>
            <p className="text-xs text-muted-foreground">
              Next review: {formatDate(kpis.fspLicence.nextReview)}
            </p>
            <div className="flex items-center gap-1">
              <Calendar className="h-3 w-3" />
              <span className="text-xs">{kpis.fspLicence.daysUntilReview} days remaining</span>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            {(kpis.ficaAlerts.kycPending > 10 || kpis.ficaAlerts.kycExpiring > 3) && (
              <Badge
                variant="secondary"
                className="text-xs bg-amber-100 text-amber-700 hover:bg-amber-200 border-amber-200"
              >
