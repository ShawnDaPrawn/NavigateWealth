...
- **Leaving the wizard**: nothing in the flow exists server-side until the field studio is reached, so `ExitWizardDialog` gates every exit — Save as draft (creates or updates the draft envelope, which then resumes from the dashboard), Discard, or Keep editing. `wizardExitState.ts` holds the two rules: whether there is anything to lose, and whether a resumable draft can be made (a draft needs a document). `materialiseEnvelopeFromWizard({ requireSigners: false })` is the draft-save path — recipients are required to _send_, not to save.
- **Creation flow chrome**: `components/wizard/` owns the send-for-signature flow's layout — `EsignWizardShell` (header, progress rail, action bar), `EsignWizardStepper`, `EsignWizardSection`, and the step model in `esignWizardSteps.ts`. Steps (`DocumentUploadStep`, `RecipientsStepView`) render content only and declare their actions; they do not lay out a page of their own. Both entry points use it — `EsignModule` with `layout="fill"`, the client drawer's E-Sign tab with `layout="inline"` — so the flow looks the same wherever it starts. The column is wide (`max-w-[1600px]`) and each step lays itself out in two columns from `xl` up; do not narrow the column to make a step fit — give the step a second column instead.
