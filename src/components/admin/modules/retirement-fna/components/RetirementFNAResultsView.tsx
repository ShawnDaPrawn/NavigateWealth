/**
 * Retirement FNA Results View Component
 * Displays detailed retirement analysis results for advisers (Read-Only View)
...
import type { RetirementFNASession } from '../types';

interface RetirementFNAResultsViewProps {
  fna: RetirementFNASession;
  onClose?: () => void;
}
  ...
  if (!fna.results) {
    return (
      <Alert>
        ...
        <AlertDescription>
          No results available. Please calculate the analysis first.
        </AlertDescription>
      </Alert>
    );
  }

  const { results, inputs, adjustments = {} } = fna;
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <div>
          <AlertDescription
            className={
              hasShortfall ? 'text-yellow-900 font-semibold' : 'text-green-900 font-semibold'
            }
          >
            {hasShortfall
          ...
          <div className="text-xs mt-1 opacity-80 pl-6">
            Published on {new Date(fna.updatedAt).toLocaleDateString()} • Version {fna.version}
          </div>
        </div>
      </Alert>
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <CardDescription className="mt-2">
                  Final calculated values based on published analysis
                </CardDescription>
              </div>
              <Badge
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-muted/50 p-4 rounded-lg">
                ...
                <p className="text-xs text-muted-foreground">
                  From current savings + contributions
                </p>
              </div>
              <div
                className={`p-4 rounded-lg ${hasShortfall ? 'bg-yellow-50 border border-yellow-200' : 'bg-green-50 border border-green-200'}`}
              >
                ...
                <Card className="border-yellow-200 bg-yellow-50/50">
                  <CardContent className="pt-6">
                    <div className="space-y-3">
                      <p className="text-sm">
                        Based on the analysis, the client requires an{' '}
                        <strong>additional monthly contribution</strong> to meet their retirement
                        goals:
                      </p>
                      ...
                      <div className="text-sm text-muted-foreground">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {formatCurrency(inputs.currentMonthlyContribution || 0)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-green-800">
                          Current savings and contribution levels are projected to meet or exceed
                          the required retirement capital. The client is well-positioned to achieve
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {((adjustments.inflationRate || 0.06) * 100).toFixed(1)}%
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Pre-Ret Growth:</span>
                  <span className="font-medium">
                    {((adjustments.preRetirementReturn || 0.1) * 100).toFixed(1)}%
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Post-Ret Growth:</span>
                  <span className="font-medium">
                    {((adjustments.postRetirementReturn || 0.08) * 100).toFixed(1)}%
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Replacement Ratio:</span>
                  <span className="font-medium">
                    {((adjustments.replacementRatio || 0.75) * 100).toFixed(0)}%
                  </span>
                </div>
              </div>
            </div>
      ...
      <Alert>
        ...
        <AlertDescription className="text-sm">
          This analysis is based on standard actuarial assumptions and current legislation. Actual
          results may vary based on market conditions, legislative changes, and individual
          circumstances.
        </AlertDescription>
      </Alert>
    </div>
