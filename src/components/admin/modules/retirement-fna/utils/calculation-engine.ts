...
export function calculateRetirementFNA(
  ...
  const retirementAge =
    adjustments.retirementAge ||
    inputs.retirementAge ||
    DEFAULT_RETIREMENT_ASSUMPTIONS.retirementAge;

  const yearsToRetirement = Math.max(0, retirementAge - currentAge);
  const yearsInRetirement =
    adjustments.yearsInRetirement || DEFAULT_RETIREMENT_ASSUMPTIONS.yearsInRetirement;

  // Economic Assumptions
  const inflation = adjustments.inflationRate ?? DEFAULT_RETIREMENT_ASSUMPTIONS.inflationRate;
  const preRetReturn =
    adjustments.preRetirementReturn ?? DEFAULT_RETIREMENT_ASSUMPTIONS.preRetirementReturn;
  const postRetReturn =
    adjustments.postRetirementReturn ?? DEFAULT_RETIREMENT_ASSUMPTIONS.postRetirementReturn;
  const salaryEscalation =
    adjustments.salaryEscalation ?? DEFAULT_RETIREMENT_ASSUMPTIONS.salaryEscalation;
  const replaceRatio =
    adjustments.replacementRatio ?? DEFAULT_RETIREMENT_ASSUMPTIONS.replacementRatio;
  const premiumEscalation =
    adjustments.premiumEscalation ?? DEFAULT_RETIREMENT_ASSUMPTIONS.premiumEscalation;
