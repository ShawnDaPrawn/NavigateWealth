    ...
    <div key="page-1">
      <section className="section">
        ...
        <div className="callout" style={{ marginBottom: '4mm' }}>
          <strong>{scenarioName}</strong> prepared for <strong>{clientName}</strong>. This report
          projects retirement capital accumulation and sustainable income drawdown based on the
          inputs provided. All figures are{' '}
          {inputs.isNominal ? 'Nominal (Future Values)' : "Real (Today's Money)"}.
        </div>

        <div className="grid grid-cols-2 gap-8">
          <div>
            <h3 className="text-[9.5px] font-bold uppercase text-gray-500 mb-2 border-b border-gray-200">
              Timeline
            </h3>
            <table className="w-full mb-4">
              <tbody>
                <tr>
                  <th>Current Age</th>
                  <td className="field">{inputs.currentAge}</td>
                </tr>
                <tr>
                  <th>Retirement Age</th>
                  <td className="field">{inputs.retirementAge}</td>
                </tr>
                <tr>
                  <th>Life Expectancy</th>
                  <td className="field">{inputs.lifeExpectancyAge}</td>
                </tr>
                <tr>
                  <th>Time Horizon</th>
                  <td className="field">
                    {results.yearsToRetirement} years to retirement + {results.yearsInRetirement}{' '}
                    years in retirement
                  </td>
                </tr>
              </tbody>
          ...
          <div>
            ...
            <table className="w-full mb-4">
              <tbody>
                <tr>
                  <th>Investment Return (Nominal)</th>
                  <td className="field">{formatPercent(inputs.nominalReturn)}</td>
                </tr>
                <tr>
                  <th>Inflation</th>
                  <td className="field">{formatPercent(inputs.inflation)}</td>
                </tr>
                <tr>
                  <th>Total Fees</th>
                  <td className="field">{formatPercent(inputs.annualFee)}</td>
                </tr>
                <tr>
                  <th>Net Real Return</th>
                  <td className="field font-bold">{formatPercent(results.netRealReturn)}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <section className="section">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className="border border-green-200 bg-green-50/50 p-4 rounded-sm">
            <div className="text-[9px] uppercase text-green-800 font-bold mb-1">
              Projected Capital at Retirement (Age {inputs.retirementAge})
            </div>
          ...
          <div className="border border-blue-200 bg-blue-50/50 p-4 rounded-sm">
            <div className="text-[9px] uppercase text-blue-800 font-bold mb-1">
          ...
          <tbody>
            <tr>
              ...
              <td className="text-right">
                {formatCurrency(results.projectionData[0]?.openingBalance || 0)}
              </td>
            </tr>
            <tr>
              <td>Retirement</td>
            ...
            <tr>
              ...
              <td>
                {new Date().getFullYear() + results.yearsToRetirement + results.yearsInRetirement}
              </td>
              <td className="text-right">
                {formatCurrency(
                  results.projectionData[results.projectionData.length - 1]?.closingBalance || 0,
                )}
              </td>
            </tr>
          </tbody>
      ...
      <div key={`page-proj-${i}`}>
        <section className="section">
          <div className="section-head">
            ...
            <h2>
              Year-by-Year Projection {tablePagesCount > 1 ? `(${i + 1}/${tablePagesCount})` : ''}
            </h2>
          </div>
            ...
            <tbody>
              {pageData.map((row, idx) => (
                <tr key={idx} className={row.phase === 'drawdown' ? 'bg-orange-50/30' : ''}>
                  ...
                  <td
                    className={`text-right ${row.contributionsOrIncome < 0 ? 'text-red-600' : 'text-green-600'}`}
                  >
