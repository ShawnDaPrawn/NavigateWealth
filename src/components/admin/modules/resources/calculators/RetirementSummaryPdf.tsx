...
export const RetirementSummaryPdf = ({
  inputs,
  results,
  clientName,
    ...
    <div className="space-y-6">
      <div className="section">
        ...
        <div className="text-[9.5px] leading-relaxed text-justify text-gray-600 mb-4">
          This retirement planning analysis projects your future capital accumulation and
          sustainable income based on your current inputs and assumptions. The values shown are in{' '}
          {inputs.isNominal
            ? 'Nominal terms (future value including inflation)'
            : "Real terms (today's buying power)"}
          .
        </div>

        <div className="grid grid-cols-2 gap-8">
          <div className="bg-gray-50/50 p-4 border border-gray-200 rounded-sm">
            <h3 className="text-[10px] font-bold uppercase tracking-wider text-blue-800 mb-3 border-b border-blue-100 pb-1">
              Retirement Goal
            </h3>
          ...
          <div className="bg-gray-50/50 p-4 border border-gray-200 rounded-sm">
            <h3 className="text-[10px] font-bold uppercase tracking-wider text-green-800 mb-3 border-b border-green-100 pb-1">
              Projected Outcome
            </h3>
            <div className="space-y-2">
              <div className="flex justify-between items-baseline">
      ...
      <div className="section">
        ...
        <div className="grid grid-cols-3 gap-6">
          <div>
            <div className="text-[9px] font-bold text-gray-700 mb-2 bg-gray-50 p-1">
              Current Position
            </div>
            <div className="space-y-1">
              ...
              <div className="flex justify-between border-b border-gray-100 pb-1">
                ...
                <span className="text-[9px] font-medium">
                  {formatCurrency(inputs.contributionAmount)} ({inputs.contributionFrequency})
                </span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between border-b border-gray-100 pb-1">
                ...
                <span className="text-[9px] font-medium">
                  {formatPercent(inputs.nominalReturn)}
                </span>
              </div>
              <div className="flex justify-between border-b border-gray-100 pb-1">
          ...
          <div>
            ...
            <div className="space-y-1">
              ...
              <div className="flex justify-between border-b border-gray-100 pb-1">
                ...
                <span className="text-[9px] font-medium">
                  {formatPercent(results.netRealReturn)}
                </span>
              </div>
            </div>
          </div>
        ...
        <table className="w-full text-[9.5px] border-collapse mt-2">
          ...
          <tbody>
            <tr>
              ...
              <td className="border border-gray-200 px-2 py-1 text-right">
                {formatPercent((results.fvCurrentSavings / results.totalCapital) * 100)}
              </td>
            </tr>
            <tr>
              <td className="border border-gray-200 px-2 py-1">Future Contributions Growth</td>
              <td className="border border-gray-200 px-2 py-1 text-right">
                {formatCurrency(results.fvContributions)}
              </td>
              <td className="border border-gray-200 px-2 py-1 text-right">
                {formatPercent((results.fvContributions / results.totalCapital) * 100)}
              </td>
            </tr>
            <tr className="bg-gray-100 font-bold">
              <td className="border border-gray-200 px-2 py-1">
                Total Capital at Age {inputs.retirementAge}
              </td>
              <td className="border border-gray-200 px-2 py-1 text-right">
                {formatCurrency(results.totalCapital)}
              </td>
      ...
      <div className="mt-8 border-t border-gray-200 pt-4">
        <div className="text-[8px] text-gray-500 italic text-justify">
          Disclaimer: This calculation is for illustrative purposes only and does not constitute
          financial advice. The projections are based on the assumptions provided and actual results
          may vary. Inflation and market returns are unpredictable.
        </div>
      </div>
    </div>,
    ...
    ...projectionPages.map((pageRows, pageIndex) => (
      <div className="space-y-4" key={`page-${pageIndex}`}>
        <div className="section-head">
          ...
          <h2>
            Year-by-Year Projection {projectionPages.length > 1 ? `(Part ${pageIndex + 1})` : ''}
          </h2>
        </div>
            ...
            {pageRows.map((row, i) => (
              <tr key={i} className={row.phase === 'drawdown' ? 'bg-orange-50/30' : ''}>
                ...
                <td className="border border-gray-200 px-2 py-1 text-center text-gray-500">
                  {new Date().getFullYear() + row.year}
                </td>
                <td className="border border-gray-200 px-2 py-1 text-right">
                  {formatCurrency(row.openingBalance)}
                </td>
                <td
                  className={`border border-gray-200 px-2 py-1 text-right ${row.contributionsOrIncome < 0 ? 'text-red-600' : 'text-green-600'}`}
                >
