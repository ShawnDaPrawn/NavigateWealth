...
export function GoalFormDialog({
  ...
  const voluntaryPolicies = policies.filter(
    (p) =>
  ...
  const togglePolicy = (id: string) => {
    ...
    setSelectedPolicyIds((prev) => (isSelecting ? [...prev, id] : prev.filter((p) => p !== id)));
  ...
  const addAdHoc = () => {
    if (newAdHocAmount && newAdHocDate) {
      setAdHocContributions([
        ...adHocContributions,
        { amount: Number(newAdHocAmount), date: newAdHocDate },
      ]);
      setNewAdHocAmount('');
      setNewAdHocDate('');
    }
        ...
        <Tabs defaultValue="details" className="w-full">
          ...
          <TabsContent value="financials" className="space-y-6 py-4">
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
              ...
              {selectedPolicyIds.map((id) => {
                ...
                return (
                  <div
                    key={id}
                    className="grid grid-cols-3 gap-0 text-sm border-b border-gray-100 bg-white hover:bg-gray-50 transition-colors"
                  >
            ...
            <div className="grid grid-cols-3 gap-4 border-t pt-4">
              <div>
                ...
                />
                {errors.growthRate && (
                  <p className="text-[10px] text-red-500 mt-1">{errors.growthRate}</p>
                )}
              </div>
              <div>
                ...
                <Input
                  ...
                  placeholder="6"
                  className={
                    errors.escalationRate ? 'border-red-500 focus-visible:ring-red-500' : ''
                  }
                />
                {errors.escalationRate && (
                  <p className="text-[10px] text-red-500 mt-1">{errors.escalationRate}</p>
                )}
              </div>
              <div>
                ...
                <Input
                  ...
                  placeholder="6"
                  className={
                    errors.inflationRate ? 'border-red-500 focus-visible:ring-red-500' : ''
                  }
                />
                {errors.inflationRate && (
                  <p className="text-[10px] text-red-500 mt-1">{errors.inflationRate}</p>
                )}
              </div>
            </div>
            ...
            <div className="border-t pt-4">
              ...
              <div className="flex gap-2 mb-2">
                ...
                <Button
                  ...
                  onClick={addAdHoc}
                  variant="outline"
                  aria-label="Add ad-hoc contribution"
                >
              ...
              {adHocContributions.length > 0 && (
                <div className="space-y-2 mt-2 max-h-[100px] overflow-y-auto">
                  {adHocContributions.map((adhoc, idx) => (
                    ...
                    >
                      <span>
                        {formatCurrency(adhoc.amount)} on {adhoc.date}
                      </span>
                      <button
          ...
          <TabsContent value="linking" className="space-y-4 py-4">
            <div className="border rounded-md p-2 max-h-[300px] overflow-y-auto space-y-2">
              {voluntaryPolicies.length === 0 ? (
                <p className="text-sm text-gray-500 italic p-4 text-center">
                  No voluntary investments available to link.
                </p>
              ) : (
                voluntaryPolicies.map((policy) => (
                  <div
                    key={policy.id}
                    className="flex items-center space-x-3 p-3 hover:bg-gray-50 rounded-lg border border-transparent hover:border-gray-200 transition-all"
                  >
                    <Checkbox
                      id={policy.id}
                    ...
                    <label htmlFor={policy.id} className="text-sm cursor-pointer flex-1">
                      <div className="flex justify-between font-medium text-gray-900">
                        <span className="flex items-center gap-2">
                          ...
                          <span className="text-gray-700">{getPolicyNumber(policy)}</span>
                        </span>
            ...
            <p className="text-xs text-gray-500">
              Linked policies are automatically included in the goal calculation using their current
              value and monthly premiums.
            </p>
          </TabsContent>
        </Tabs>
