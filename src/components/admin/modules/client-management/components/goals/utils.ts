...
export function calculatePolicyFV(
  ...
  if (monthlyContribution > 0) {
    if (Math.abs(r - e) < 0.0000001) {
      ...
      fvContributions =
        monthlyContribution *
        ((Math.pow(1 + r, monthsToTarget) - Math.pow(1 + e, monthsToTarget)) / (r - e));
    }
  }
...
export function calculateGoalStatus(
  ...
  const getPolicyValue = (p: GoalPolicyRecord) => {
    return (
      cleanCurrencyValue(p.data?.invest_current_value) ||
  ...
  const getPolicyPremium = (p: GoalPolicyRecord) => {
    return (
      cleanCurrencyValue(p.data?.invest_monthly_contribution) ||
