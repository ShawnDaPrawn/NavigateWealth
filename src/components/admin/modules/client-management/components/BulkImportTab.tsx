...
const TEMPLATE_COLUMNS = [
  ...
  { header: 'Tax Number', field: 'taxNumber', required: false },
  {
    ...
    field: 'maritalStatus',
    required: false,
    hint: 'Single, Married, Divorced, Widowed, Life Partner',
  },
  ...
  { header: 'Postal Code', field: 'residentialPostalCode', required: false },
  {
    ...
    required: false,
    hint: 'Default: South Africa',
  },
  {
    header: 'Employment Status',
    field: 'employmentStatus',
    required: false,
    hint: 'employed, self-employed, unemployed, retired, student',
  },
...
function parseExcel(file: File): Promise<ParsedRow[]> {
  return new Promise((resolve, reject) => {
    if (file.size > MAX_BULK_IMPORT_BYTES) {
      reject(
        new Error(
          `The spreadsheet is too large. Please upload a file smaller than ${MAX_BULK_IMPORT_FILE_MB} MB.`,
        ),
      );
      return;
    }
    ...
    reader.onload = (e) => {
      try {
        ...
        if (raw.length < 2) {
          reject(
            new Error(
              'The spreadsheet has no data rows. Please add client data below the header row.',
            ),
          );
          return;
        }
        ...
        if (raw.length > MAX_BULK_IMPORT_ROWS + 1) {
          reject(
            new Error(
              `Maximum ${MAX_BULK_IMPORT_ROWS} clients per import. Please split your file.`,
            ),
          );
          return;
        }
...
export function BulkImportTab({ onSuccess, onClose }: BulkImportTabProps) {
  ...
  const handleImport = async () => {
    ...
    if (validRows.length > MAX_BULK_IMPORT_ROWS) {
      toast.error(`Maximum ${MAX_BULK_IMPORT_ROWS} clients per import. Please split your file.`);
      return;
    }
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50/80">
                  <TableHead className="w-14 text-[11px] font-semibold uppercase tracking-wider text-gray-500">
                    Row
                  </TableHead>
                  <TableHead className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">
                    Name
                  </TableHead>
                  <TableHead className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">
                    Email
                  </TableHead>
                  <TableHead className="w-28 text-[11px] font-semibold uppercase tracking-wider text-gray-500">
                    Status
                  </TableHead>
                  <TableHead className="text-[11px] font-semibold uppercase tracking-wider text-gray-500">
                    App #
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Pre-formatted Excel file with all supported fields. Required:{' '}
              <span className="font-medium text-gray-600">
                First Name, Last Name, Email, Cellphone
              </span>
              .
            </p>
          </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {fileName ? (
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                Maximum {MAX_BULK_IMPORT_ROWS.toLocaleString()} clients per upload. Accepts{' '}
                <code className="text-[10px] bg-gray-100 px-1 py-0.5 rounded">.xlsx</code> files
                only.
              </p>
            )}
          </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-gray-50/80">
                    ...
                    <TableHead className="w-20 text-[11px] font-semibold uppercase tracking-wider text-gray-500 text-center">
                      Valid
                    </TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {parsedRows.map((row) => (
                    <TableRow key={row.rowNum} className={row.valid ? '' : 'bg-red-50/40'}>
                      ...
                      <TableCell className="text-sm text-gray-500">
                        {row.data.emailAddress}
                      </TableCell>
                      <TableCell className="text-sm text-gray-500">
                        {row.data.cellphoneNumber}
                      </TableCell>
            ...
            {invalidCount > 0 && (
              <div className="mx-4 my-3 flex items-start gap-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                ...
                <p className="text-xs text-red-700">
                  ...
                  with validation errors will be skipped. Only <strong>{validCount}</strong> valid
                  row{validCount !== 1 ? 's' : ''} will be imported.
                </p>
              </div>
            )}
