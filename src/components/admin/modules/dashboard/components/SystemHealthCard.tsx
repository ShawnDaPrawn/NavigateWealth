 ...
 *
 * Shows the platform's maintenance status: last KV cleanup run timestamp,
 * keys purged, and duration. Includes a "Run Cleanup" button that follows
 * the dry-run-first pattern (Guidelines §14.1): preview first, then confirm
 * via AlertDialog before executing a live deletion.
 *
 * `SystemHealthPanel` holds all state and UI so it can be embedded either
 * in the legacy `SystemHealthCard` or in the dashboard's `SystemHealthDialog`.
 *
 * Guidelines:
...
/**
 * The system-health body: status banner, last-run stats, and the
 * dry-run-first cleanup flow. Fetches data on mount, so embedding it in a
 * dialog means no requests are made until the dialog opens.
 */
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs text-muted-foreground">
          Scan the KV store for stale entries and purge them safely.
        </p>
        <Button
          variant="outline"
          size="sm"
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <p className="text-sm font-medium">{stale ? 'Cleanup overdue' : 'System healthy'}</p>
              <Badge
                variant="outline"
              ...
              {previewResult.totalKeysFound === 0 ? (
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2 pt-1">
                    ...
                    <Button size="sm" variant="ghost" onClick={resetStage} className="h-7 text-xs">
                      Cancel
                    </Button>
                  </div>
                </div>
