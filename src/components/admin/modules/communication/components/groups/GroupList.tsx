        ...
        <Card key={group.id} className="relative hover:shadow-md transition-shadow">
          ...
          <CardContent>
            ...
            <div className="flex flex-wrap gap-1 mb-4">
              {group.filterConfig?.productFilters &&
                group.filterConfig.productFilters.length > 0 && (
                  <Badge variant="secondary" className="text-[10px]">
                    Product Rules
                  </Badge>
                )}
              {group.filterConfig?.netWorthFilters &&
                group.filterConfig.netWorthFilters.length > 0 && (
                  <Badge variant="secondary" className="text-[10px]">
                    Net Worth Rules
                  </Badge>
                )}
              ...
              {group.filterConfig?.dependantCountFilters &&
                group.filterConfig.dependantCountFilters.length > 0 && (
                  <Badge variant="secondary" className="text-[10px]">
                    Dependants
                  </Badge>
                )}
              {group.filterConfig?.retirementAgeFilters &&
                group.filterConfig.retirementAgeFilters.length > 0 && (
                  <Badge variant="secondary" className="text-[10px]">
                    Retirement Age
                  </Badge>
                )}
            ...
            {onSelectGroup && (
              <Button variant="secondary" className="w-full" onClick={() => onSelectGroup(group)}>
                Use This Group
              </Button>
            )}
          </CardContent>
