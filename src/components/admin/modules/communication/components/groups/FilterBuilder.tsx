  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Clients matching these rules will be automatically included.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-8">
        ...
        <div className="space-y-3">
          ...
          {(filterConfig.productFilters || []).length === 0 && (
            <p className="text-sm text-muted-foreground italic bg-muted/10 p-2 rounded border border-dashed">
              No product filters applied. (Clients with ANY products included)
            </p>
          )}
          ...
          {(filterConfig.netWorthFilters || []).map((filter, index) => (
            <div key={index} className="flex items-center gap-4 p-3 border rounded-md bg-muted/20">
              ...
              <Button
                variant="ghost"
                size="icon"
                onClick={() => removeNetWorthFilter(index)}
                className="mt-4"
                aria-label="Remove net worth filter"
              >
                <Trash2 className="h-4 w-4 text-muted-foreground hover:text-destructive" />
              </Button>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {MARITAL_STATUS_OPTIONS.map((status) => (
                <Badge
                  key={status}
                  variant={
                    filterConfig.maritalStatusFilters?.includes(status) ? 'default' : 'outline'
                  }
            ...
            <div className="flex flex-wrap gap-2">
              {EMPLOYMENT_STATUS_OPTIONS.map((status) => (
                <Badge
                  key={status}
                  variant={
                    filterConfig.employmentStatusFilters?.includes(status) ? 'default' : 'outline'
                  }
                  className="cursor-pointer hover:bg-primary/90 hover:text-primary-foreground"
          ...
          {(filterConfig.incomeFilters || []).map((filter, index) => (
            <div key={index} className="flex items-center gap-4 p-3 border rounded-md bg-muted/20">
              ...
              <Button
                variant="ghost"
                size="icon"
                onClick={() => removeIncomeFilter(index)}
                className="mt-4"
          ...
          <div className="flex flex-wrap gap-2">
            {OCCUPATION_OPTIONS.map((occupation) => (
              <Badge
                key={occupation}
                variant={
                  filterConfig.occupationFilters?.includes(occupation) ? 'default' : 'outline'
                }
                className="cursor-pointer hover:bg-primary/90 hover:text-primary-foreground"
        ...
        <div className="space-y-3">
          ...
          {(filterConfig.dependantCountFilters || []).length === 0 && (
            <p className="text-sm text-muted-foreground italic bg-muted/10 p-2 rounded border border-dashed">
              No dependant count filters applied.
            </p>
          )}
          ...
          {(filterConfig.dependantCountFilters || []).map((filter, index) => (
            <div key={index} className="flex items-center gap-4 p-3 border rounded-md bg-muted/20">
              ...
              <Button
                variant="ghost"
                size="icon"
                onClick={() => removeDependantCountFilter(index)}
                className="mt-4"
                aria-label="Remove dependant count filter"
              >
                <Trash2 className="h-4 w-4 text-muted-foreground hover:text-destructive" />
              </Button>
            </div>
        ...
        <div className="space-y-3">
          ...
          {(filterConfig.retirementAgeFilters || []).length === 0 && (
            <p className="text-sm text-muted-foreground italic bg-muted/10 p-2 rounded border border-dashed">
              No retirement age filters applied.
            </p>
          )}
          ...
          {(filterConfig.retirementAgeFilters || []).map((filter, index) => (
            <div key={index} className="flex items-center gap-4 p-3 border rounded-md bg-muted/20">
              ...
              <Button
                variant="ghost"
                size="icon"
                onClick={() => removeRetirementAgeFilter(index)}
                className="mt-4"
                aria-label="Remove retirement age filter"
              >
                <Trash2 className="h-4 w-4 text-muted-foreground hover:text-destructive" />
              </Button>
            </div>
