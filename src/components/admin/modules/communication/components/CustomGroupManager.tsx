...
export function CustomGroupManager({ onClose, onSelectGroup }: CustomGroupManagerProps) {
  ...
  const loadData = async () => {
    try {
      ...
      // Include custom groups, the newsletter system group (editable external
      // contacts), and auto-generated provider groups (read-only, but visible
      // so admins can see and use them).
      setGroups(
        fetchedGroups.filter(
          (g) =>
            !g.type || g.type === 'custom' || g.id === 'sys_newsletter_contacts' || g.autoGenerated,
        ),
      );
          ...
          <Table>
            ...
            <TableBody>
              {filteredGroups.map((group) => {
                ...
                const isAutoGroup = !!group.autoGenerated;
                  ...
                  <TableRow key={group.id} className="cursor-pointer hover:bg-muted/50">
                    <TableCell className="font-medium">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {isAutoGroup ? 'auto' : isSystemGroup ? 'system' : 'custom'}
                        </Badge>
                      </div>
                    </TableCell>
