...
export function MedicalFNAResultsView({ results: propResults, fna }: MedicalFNAResultsViewProps) {
  ...
  const calculations: MedicalFNAResults | undefined =
    session?.calculations ||
    session?.results ||
    (pr?.rationale ? (pr as unknown as MedicalFNAResults) : undefined);
  ...
  const getMsaGap = () => {
    // If inputs are missing, we can't calculate gap
    if (!inputs)
      return { status: 'Unknown', color: 'text-gray-500', bg: 'bg-gray-100', icon: Check, msg: '' };
