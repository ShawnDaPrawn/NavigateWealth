...
export function StepReviewLivingWill({
  personalDetails: pd,
  livingWillData,
}: StepReviewLivingWillProps) {
  return (
    <div className="space-y-5">
      ...
      <ReviewSection icon={User} title="Personal Details" iconColor="text-[#6d28d9]">
        <div className="space-y-0.5">
          ...
          <ReviewRow
            ...
            value={MARITAL_STATUS_LABELS[pd.maritalStatus] || pd.maritalStatus}
          />
      ...
      <ReviewSection
        icon={Shield}
        title={`Healthcare Agents (${livingWillData.healthcareAgents.length})`}
              ...
              <div key={agent.id} className="flex items-start gap-3 py-2">
                ...
                <div>
                  <p className="font-medium text-gray-900">
                    {agent.name}
                    {agent.isPrimary && (
                      <Badge className="ml-2 text-xs bg-[#6d28d9] hover:bg-[#5b21b6]">
                        Primary
                      </Badge>
                    )}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {agent.relationship}
                    {agent.contactDetails ? ` | ${agent.contactDetails}` : ''}
                  </p>
                </div>
              </div>
            ))}
      ...
      <ReviewSection icon={Activity} title="Treatment Preferences" iconColor="text-amber-600">
        <div className="space-y-2">
          {(['ventilator', 'cpr', 'artificialNutrition', 'dialysis', 'antibiotics'] as const).map(
            (treatment) => {
      ...
      <ReviewSection icon={Stethoscope} title="Pain Management" iconColor="text-teal-600">
        <div className="space-y-0.5">
          <ReviewRow
            ...
            value={livingWillData.painManagement.comfortCareOnly ? 'Yes' : 'No'}
          />
          <ReviewRow
            label="Maximum Pain Relief"
            value={livingWillData.painManagement.maximumPainRelief ? 'Yes' : 'No'}
          />
        </div>
      </ReviewSection>
      ...
      <ReviewSection icon={HandHeart} title="Organ Donation" iconColor="text-pink-600">
        <div className="space-y-0.5">
          <ReviewRow
            label="Organ Donor"
            value={livingWillData.organDonation.isDonor ? 'Yes' : 'No'}
          />
          {livingWillData.organDonation.isDonor && (
            <ReviewRow
              label="Donation Type"
              value={
                livingWillData.organDonation.donationType === 'all'
  ...
  return (
    <div className="space-y-5">
        ...
        {beneficiaries.length === 0 ? (
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between font-medium">
              <span>Total</span>
              <span
                className={`font-mono ${beneficiaryTotal === 100 ? 'text-green-600' : 'text-red-600'}`}
              >
                {beneficiaryTotal}%
              </span>
            </div>
      ...
      {specificBequests.length > 0 && (
        <ReviewSection
          icon={Home}
          title={`Specific Bequests (${specificBequests.length})`}
