  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (Array.isArray(value)) {
      ...
      if (value.every((v) => typeof v === 'string')) {
        ...
        value.every((v) => typeof v === 'object' && v !== null && ('dob' in v || 'age' in v))
      ) {
        const childStr = value
          .map((child, i) => {
            const c = child as Record<string, unknown>;
            if (c.dob) {
              try {
      ...
      if ('selected' in vObj && 'adviser_assist' in vObj) {
        if (vObj.selected) {
          ...
          if (vObj.adviser_assist) val = 'Adviser assistance requested';
            ...
            val = `R${Math.round(Number(vObj.amount))
              .toString()
              .replace(/\B(?=(\d{3})+(?!\d))/g, ',')}`;
          else if (vObj.amount_per_month)
            val = `R${Math.round(Number(vObj.amount_per_month))
              .toString()
    ...
    if (typeof value === 'boolean') displayValue = value ? 'Yes' : 'No';
      ...
      displayValue = `R${Math.round(value)
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ',')}`;
    }
