...
export function Step2SystemCalculation({ inputs, calculations, onNext, onBack }: Step2Props) {
  ...
  const tfsaUtilisation = safePercent(
    inputs.tfsaContributionsLifetime,
    TAX_YEAR_2026_2027.TFSA_LIFETIME_LIMIT,
  );

  const totalRebates =
    calculations.primaryRebate + calculations.secondaryRebate + calculations.tertiaryRebate;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 flex gap-3 items-center text-slate-800">
        ...
        <div>
          ...
          <p className="text-sm text-slate-600">
            Deterministic projection based on 2026/2027 tax tables. This is a baseline calculation
            without adjustments.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="bg-gradient-to-br from-white to-slate-50">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-slate-900">
              {formatMoney(calculations.totalTaxLiability)}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              Includes Income Tax, Dividend Tax &amp; CGT
            </p>
          </CardContent>
        </Card>
        ...
        <Card
          className={`bg-gradient-to-br from-white ${calculations.raGap > 0 ? 'to-amber-50 border-amber-200' : 'to-green-50'}`}
        >
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground uppercase tracking-wide">
              RA Deduction Gap
          ...
          <CardContent>
            <div
              className={`text-3xl font-bold ${calculations.raGap > 0 ? 'text-amber-600' : 'text-green-600'}`}
            >
            ...
            <div className="divide-y text-sm">
              ...
              <div className="py-3 flex justify-between items-center bg-green-50/50 -mx-4 px-4 text-green-700">
                <span className="flex items-center gap-2">
                  3. Less: Retirement Annuity Deduction
                </span>
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span
                    className={calculations.taxableInterest > 0 ? 'text-red-500' : 'text-green-600'}
                  >
                ...
                {calculations.taxableInterest > 0 && (
                  <div className="bg-red-50 p-2 rounded text-xs text-red-700">
                    <span className="font-bold">Leakage:</span> You are paying marginal tax on{' '}
                    {formatMoney(calculations.taxableInterest)} of interest.
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-green-600">
                    {formatMoney(calculations.medicalTaxCredits)}/yr
                  </span>
                </div>
                <p className="text-xs text-muted-foreground">
                  {inputs.medicalSchemeMembers} member{inputs.medicalSchemeMembers !== 1 ? 's' : ''}{' '}
                  on scheme
                </p>
              </div>
            </CardContent>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
            ...
            <p className="text-xs text-amber-700 leading-relaxed">
              These figures are mathematically generated based on confirmed inputs and 2026/2027
              SARS tables. In the next step, you can apply manual adjustments for once-off events or
              specific exclusions.
            </p>
          </div>
        </div>
