        ...
        <div className="col-span-1 flex justify-end">
          {isModified && (
            <Button
              ...
              aria-label={`Reset ${String(field)} to calculated value`}
            >
        ...
        <div className="flex gap-3 items-center text-amber-900">
          ...
          <div>
            ...
            <p className="text-sm text-amber-800">
              Modify inputs below to create an "Adjusted Scenario". All changes are logged for
              compliance.
            </p>
          </div>
        </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y">
                  <tr>
                    ...
                    <td className="text-right p-3 bg-amber-50/50 font-semibold">
                      {formatMoney(adjustedResults.taxableIncome)}
                    </td>
                  ...
                  <tr>
                    ...
                    <td className="text-right p-3">
                      {formatMoney(baselineResults.totalTaxLiability)}
                    </td>
                    <td className="text-right p-3 bg-amber-50/50 font-bold text-slate-900">
                      {formatMoney(adjustedResults.totalTaxLiability)}
                    </td>
                    <td
                      className={`text-right p-3 pr-6 font-medium ${adjustedResults.totalTaxLiability < baselineResults.totalTaxLiability ? 'text-green-600' : 'text-red-600'}`}
                    >
                      {formatMoney(
                        adjustedResults.totalTaxLiability - baselineResults.totalTaxLiability,
                      )}
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td className="text-right p-3">
                      {(baselineResults.effectiveTaxRate * 100).toFixed(1)}%
                    </td>
                    <td className="text-right p-3 bg-amber-50/50 font-semibold">
                      {(adjustedResults.effectiveTaxRate * 100).toFixed(1)}%
                    </td>
                  ...
                  <tr>
                    ...
                    <td className="text-right p-3 bg-amber-50/50">
                      {formatMoney(adjustedResults.raGap)}
                    </td>
                    <td className="text-right p-3 pr-6 text-slate-400">
                  ...
                  <tr>
                    ...
                    <td className="text-right p-3">
                      {formatMoney(baselineResults.medicalTaxCredits)}
                    </td>
                    <td className="text-right p-3 bg-amber-50/50">
                      {formatMoney(adjustedResults.medicalTaxCredits)}
                    </td>
                    <td className="text-right p-3 pr-6 text-slate-400">
                      {formatMoney(
                        adjustedResults.medicalTaxCredits - baselineResults.medicalTaxCredits,
                      )}
                    </td>
                  </tr>
                </tbody>
            ...
            <Card className="border-amber-200 bg-amber-50">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm text-amber-900">
                  Audit Trail ({adjustments.length} changes)
                </CardTitle>
              </CardHeader>
              <CardContent>
