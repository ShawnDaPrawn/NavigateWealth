    ...
    return (
      <Card>
        <CardContent className="p-6">
          <div className="text-center text-muted-foreground">No tax plan data available.</div>
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div className="flex justify-between items-start">
        ...
        <Badge variant="outline" className="text-slate-600 border-slate-300">
          {((inputs as unknown as Record<string, unknown>).taxYear as string) ||
            `${new Date(generatedAt).getFullYear()}/${new Date(generatedAt).getFullYear() + 1}`}{' '}
          Tax Year
        </Badge>
      </div>
        ...
        <Card className="bg-slate-50 border-slate-200">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-slate-900">
              {formatMoney(finalResults.grossIncome)}
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-50 border-slate-200">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-slate-900">
              {formatMoney(finalResults.totalTaxLiability)}
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-50 border-slate-200">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-slate-900">
              {formatPercent(finalResults.effectiveTaxRate)}
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="pt-6 space-y-6">
          {acceptedRecs.length === 0 ? (
            <p className="text-slate-500 italic">
              No specific recommendations were included in this record.
            </p>
          ) : (
        ...
        <CardContent className="space-y-0">
          <div className="divide-y text-sm">
            ...
            <div className="py-2 flex justify-between">
              ...
              <span>
                {formatMoney(inputs.businessIncome + inputs.rentalIncome + inputs.foreignIncome)}
              </span>
            </div>
            ...
            <div className="py-2 flex justify-between text-slate-500">
              ...
              <span>
                - {formatMoney(finalResults.interestExemption + finalResults.actualRADeduction)}
              </span>
            </div>
            ...
            <div className="py-2 flex justify-between text-green-600">
              ...
              <span>
                -{' '}
                {formatMoney(
                  finalResults.primaryRebate +
                    finalResults.secondaryRebate +
                    finalResults.tertiaryRebate,
                )}
              </span>
            </div>
        ...
        <Card className="bg-amber-50 border-amber-200">
          ...
          <CardContent className="space-y-4">
            {adjustments.length > 0 && (
              <div>
                <h5 className="text-xs font-bold text-amber-800 uppercase mb-2">
                ...
                <ul className="space-y-1">
                  {adjustments.map((adj) => (
                    <li
                      key={adj.id}
                      className="text-sm text-amber-900 flex justify-between border-b border-amber-200/50 pb-1 last:border-0"
                    >
                      <span>{adj.field}</span>
                      <span className="font-mono text-xs">
                        {formatMoney(Number(adj.originalValue))} &rarr;{' '}
