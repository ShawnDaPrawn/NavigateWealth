...
interface BankingSectionProps {
  ...
  removeProofOfBank: () => void;
  setBankAccountToDelete: (value: string | null) => void;
  setProofOfBankToDelete: (value: string | null) => void;
...
export function BankingSection({
  ...
  return (
    <div className="contents">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>Client's bank account information</CardDescription>
              </div>
            </div>
            <Button
              onClick={addBankAccount}
              size="sm"
              disabled={bankAccountsInEditMode.size > 0}
              className="bg-[#6d28d9] hover:bg-[#5b21b6] disabled:opacity-50 disabled:cursor-not-allowed"
              title={bankAccountsInEditMode.size > 0 ? "Please save the current bank account before adding a new one" : "Add a new bank account"}
            >
        ...
        <CardContent>
          {profileData.bankAccounts.length === 0 ? (
            <EmptyState
              ...
              description={emptyStateConfigs.banking.description}
              actionLabel={emptyStateConfigs.banking.actionLabel}
              onAction={addBankAccount}
              iconColor={emptyStateConfigs.banking.iconColor}
              iconBgColor={emptyStateConfigs.banking.iconBgColor}
              buttonColor={emptyStateConfigs.banking.buttonColor}
              buttonHoverColor={emptyStateConfigs.banking.buttonHoverColor}
            />
          ) : (
            <div className="space-y-4">
              {profileData.bankAccounts.map((account: BankAccount, index: number) => {
                ...
                if (isOtherBank) {
                  ...
                  isValid = isValid && !!account.branchCode;
                }
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <p className="text-gray-900">
                              {account.accountHolderName || `Bank Account ${index + 1}`}
                            </p>
                            {account.isPrimary && (
                              <Badge className="bg-[#6d28d9] text-white">Primary</Badge>
                            )}
                          </div>
                          {!isInEditMode && account.bankName && (
                            <p className="text-sm text-gray-600">
                              {isOtherBank ? account.customBankName : account.bankName} • {account.accountType}
                            </p>
                          )}
                        </div>
                      </div>
                        ...
                        {!isInEditMode ? (
                          <div className="contents">
                            ...
                            {profileData.bankAccounts.length > 1 && (
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => confirmDeleteBankAccount(account.id)}
                          ...
                          <div className="contents">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => cancelEditBankAccount(account.id)}
                    ...
                    {isInEditMode ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="sm:col-span-2">
                          <Label htmlFor={`holder-${account.id}`}>Account Holder Name *</Label>
                          <Input
                        ...
                        <div className="sm:col-span-2">
                          ...
                          >
                            <SelectTrigger id={`bank-${account.id}`} className="mt-1.5">
                        ...
                        {/* Conditional "Other" Bank Fields */}
                        {isOtherBank && (
                          <div className="contents">
                            <div className="sm:col-span-2">
                              <Label htmlFor={`custom-bank-${account.id}`}>Custom Bank Name *</Label>
                              <Input
                                id={`custom-bank-${account.id}`}
                                value={account.customBankName || ''}
                                onChange={(e) => updateBankAccount(account.id, { customBankName: e.target.value })}
                            ...
                            <div className="sm:col-span-2">
                              <Label htmlFor={`custom-branch-${account.id}`}>Custom Branch Code *</Label>
                              <Input
                                id={`custom-branch-${account.id}`}
                                value={account.customBranchCode || ''}
                                onChange={(e) => updateBankAccount(account.id, { customBranchCode: e.target.value })}
                        ...
                        <div>
                          <Label htmlFor={`account-num-${account.id}`}>Account Number *</Label>
                          <Input
                            id={`account-num-${account.id}`}
                        ...
                        {!isOtherBank && (
                          <div>
                            <Label htmlFor={`branch-${account.id}`}>Branch Code *</Label>
                            <Input
                              id={`branch-${account.id}`}
                              value={account.branchCode}
                              onChange={(e) => updateBankAccount(account.id, { branchCode: e.target.value })}
                              placeholder="Enter branch code"
                              className="mt-1.5"
                            />
                          </div>
                        ...
                        <div className="sm:col-span-2">
                          ...
                          >
                            <SelectTrigger id={`account-type-${account.id}`} className="mt-1.5">
                          ...
                          {account.proofOfBankDocument ? (
                            <div className="mt-1.5 p-4 border-2 border-green-200 rounded-lg bg-green-50">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  ...
                                  <div>
                                    <p className="text-sm text-gray-900">{account.proofOfBankFileName}</p>
                                ...
                                <div className="flex gap-2">
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() => {
                                      const input = document.getElementById(`proof-${account.id}`) as HTMLInputElement;
                                      input?.click();
                                    }}
                                  ...
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => confirmDeleteProofOfBank(account.id)}
                            ...
                            <div className="mt-1.5 p-4 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50">
                              <div className="text-center">
                                ...
                                <p className="text-sm text-gray-600 mb-1">Upload Proof of Bank</p>
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (file) {
                                ...
                                handleProofOfBankUpload(account.id, file);
                              }
                            }}
                          />
                        </div>
                        ...
                        <div className="sm:col-span-2 flex items-center space-x-2 p-3 rounded-lg border border-gray-200 bg-gray-50">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => updateBankAccount(account.id, { isPrimary: checked as boolean })}
                          />
                          <Label htmlFor={`primary-${account.id}`} className="text-sm cursor-pointer">
                      ...
                      <div className="space-y-3">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                          <div>
                            <p className="text-gray-600 text-[12px]">Bank</p>
                            <p className="text-gray-900">{isOtherBank ? account.customBankName : account.bankName}</p>
                          </div>
                          <div>
                            <p className="text-gray-600 text-[12px]">Account Number</p>
                            <p className="text-gray-900">{account.accountNumber}</p>
                          </div>
                          <div>
                            <p className="text-gray-600 text-[12px]">Branch Code</p>
                            <p className="text-gray-900">{isOtherBank ? account.customBranchCode : account.branchCode}</p>
                          </div>
                          <div>
                            <p className="text-gray-600 text-[12px]">Account Type</p>
                            <p className="text-gray-900 capitalize">{account.accountType}</p>
                          </div>
                        </div>
                        {account.proofOfBankDocument && (
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this bank account? This action cannot be undone and all information about this account will be permanently removed.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setBankAccountToDelete(null)}>Cancel</AlertDialogCancel>
            <AlertDialogAction 
      ...
      {/* Delete Proof of Bank Document Confirmation Dialog */}
      <AlertDialog open={proofOfBankToDelete !== null} onOpenChange={() => setProofOfBankToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Proof of Bank Document</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete this proof of bank document? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setProofOfBankToDelete(null)}>Cancel</AlertDialogCancel>
            <AlertDialogAction 
              onClick={removeProofOfBank}
              className="bg-red-600 hover:bg-red-700"
            >
              Delete
            </AlertDialogAction>
