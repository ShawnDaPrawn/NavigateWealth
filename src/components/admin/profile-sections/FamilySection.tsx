...
import { Users, Plus, Edit2, Trash2, X, Check, User } from 'lucide-react';
...
interface FamilySectionProps {
  ...
  setFamilyMemberToDelete: (value: string | null) => void;
}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>Client's family and dependents</CardDescription>
              </div>
            </div>
            <Button
              onClick={addFamilyMember}
              size="sm"
              disabled={familyMembersInEditMode.size > 0}
              className="bg-[#6d28d9] hover:bg-[#5b21b6] disabled:opacity-50 disabled:cursor-not-allowed"
              title={familyMembersInEditMode.size > 0 ? "Please save the current family member before adding a new one" : "Add a new family member"}
            >
        ...
        <CardContent>
          {profileData.familyMembers.length === 0 ? (
            <EmptyState
              ...
              description={emptyStateConfigs.family.description}
              actionLabel={emptyStateConfigs.family.actionLabel}
              onAction={addFamilyMember}
              iconColor={emptyStateConfigs.family.iconColor}
              iconBgColor={emptyStateConfigs.family.iconBgColor}
              buttonColor={emptyStateConfigs.family.buttonColor}
              buttonHoverColor={emptyStateConfigs.family.buttonHoverColor}
            />
          ) : (
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {member.relationship && !isInEditMode && (
                        ...
                        {!isInEditMode ? (
                          <div className="contents">
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => confirmDeleteFamilyMember(member.id)}
                          ...
                          <div className="contents">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => cancelEditFamilyMember(member.id)}
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => saveFamilyMember(member.id)}
                              disabled={!isValid}
                    ...
                    {isInEditMode ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          >
                            <SelectTrigger id={`relationship-${member.id}`} className="mt-1.5">
                        ...
                        <div>
                          ...
                          >
                            <SelectTrigger id={`gender-${member.id}`} className="mt-1.5">
                        ...
                        <div>
                          <Label htmlFor={`id-${member.id}`}>ID/Passport Number</Label>
                          <Input
                            id={`id-${member.id}`}
                            value={member.idPassportNumber}
                            onChange={(e) => updateFamilyMember(member.id, { idPassportNumber: e.target.value })}
                        ...
                        <div className="sm:col-span-2 flex items-center space-x-2 p-3 rounded-lg border border-gray-200 bg-gray-50">
                          <Checkbox
                            ...
                            checked={member.isFinanciallyDependent}
                            onCheckedChange={(checked) => 
                              updateFamilyMember(member.id, { isFinanciallyDependent: checked as boolean })
                            }
                          />
                          <Label htmlFor={`dependent-${member.id}`} className="text-sm cursor-pointer">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                          {member.dateOfBirth && (
                            <div>
                              ...
                              <p className="text-gray-900 text-[12px]">{new Date(member.dateOfBirth).toLocaleDateString()}</p>
                            </div>
                          )}
                          {member.gender && (
                            <div>
                              <p className="text-gray-600">Gender</p>
                              <p className="text-gray-900 capitalize text-[12px]">{member.gender}</p>
                            </div>
                          )}
                          {member.idPassportNumber && (
                            <div>
                              <p className="text-gray-600">ID/Passport Number</p>
                              <p className="text-gray-900 text-[12px]">{member.idPassportNumber}</p>
                            </div>
                          )}
                          <div>
                            <p className="text-gray-600">Dependency Status</p>
                            <p className="text-gray-900 text-[12px]">{member.isFinanciallyDependent ? 'Financially Dependent' : 'Independent'}</p>
                          </div>
                        </div>
                      </div>
                    )}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this family member? This action cannot be undone and all information about this family member will be permanently removed.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setFamilyMemberToDelete(null)}>Cancel</AlertDialogCancel>
            <AlertDialogAction 
