...
export function PolicyTable({
  ...
  const getFooterColors = () => {
    switch (colorTheme) {
      case 'green':
        return { bg: 'bg-green-50/50 hover:bg-green-50/80', cell: 'bg-green-50/50' };
      case 'blue':
        return { bg: 'bg-blue-50/50 hover:bg-blue-50/80', cell: 'bg-blue-50/50' };
      case 'indigo':
        return { bg: 'bg-indigo-50/50 hover:bg-indigo-50/80', cell: 'bg-indigo-50/50' };
      case 'amber':
        return { bg: 'bg-amber-50/50 hover:bg-amber-50/80', cell: 'bg-amber-50/50' };
      case 'orange':
        return { bg: 'bg-orange-50/50 hover:bg-orange-50/80', cell: 'bg-orange-50/50' };
      case 'purple':
      default:
        return { bg: 'bg-purple-50/50 hover:bg-purple-50/80', cell: 'bg-purple-50/50' };
    }
  };
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {structure.map((field) => (
                  <TableHead key={field.id} className="min-w-[130px]">
                    {field.label || field.name || field.id}
                  </TableHead>
                ))}
                <TableHead className="text-right min-w-[100px] sticky right-0 bg-white">
                  Actions
                </TableHead>
              </TableRow>
            </TableHeader>
              ...
              {policies.map((policy) => {
                ...
                return (
                  <TableRow
                    key={policy.id}
                    className={
                      policy.archived ? 'bg-gray-50/80 text-gray-500 hover:bg-gray-50' : ''
                    }
                  >
                    <TableCell className="min-w-[150px]">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                          <Building2
                            className={`h-4 w-4 ${policy.archived ? 'text-gray-300' : 'text-gray-400'}`}
                          />
                          <span
                            className={`font-medium ${policy.archived ? 'text-gray-500 line-through' : ''}`}
                          >
                ...
                {structure.map((field) => {
                  ...
                  return (
                    <TableCell key={field.id} className="min-w-[130px] font-semibold">
                      {field.type === 'currency' && total ? `R${total.toLocaleString()}` : '-'}
                    </TableCell>
                  );
                })}
                <TableCell
                  className={`text-right min-w-[100px] sticky right-0 ${footerCellClass}`}
                ></TableCell>
              </TableRow>
            </TableFooter>
          </Table>
