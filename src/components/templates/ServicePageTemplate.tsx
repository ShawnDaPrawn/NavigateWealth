...
export function ServicePageTemplate({ config, seoData, children }: ServicePageTemplateProps) {
  ...
  const goPrevIndiv = () =>
    setActiveIndividual(indivOpts[(indivIdx - 1 + indivOpts.length) % indivOpts.length].id);
  ...
  const goPrevBiz = () =>
    setActiveBusiness(bizOpts[(bizIdx - 1 + bizOpts.length) % bizOpts.length].id);
    ...
    <div className="min-h-screen bg-white">
      ...
      {config.hero.heroStyle === 'unified' ? (
        ...
        <section
          ...
          aria-label={`${config.hero.badgeText} hero`}
        >
                  ...
                  <div className="absolute bottom-5 left-5 bg-white/90 backdrop-blur-sm rounded-2xl px-5 py-4 shadow-2xl flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-[11px] text-gray-400 font-medium uppercase tracking-wide">
                        {config.hero.statusLabel}
                      </div>
                      <div className="text-sm font-bold text-gray-900">
                        {config.hero.statusValue}
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <section
          className="relative section-dark-gray overflow-hidden"
          aria-label={`${config.hero.badgeText} hero`}
        >
          <div className="hidden lg:block absolute inset-y-0 right-0 w-[44%]">
            ...
            <div
              ...
              style={{ background: 'linear-gradient(to right, #313653 0%, transparent 100%)' }}
            />
            ...
            <div className="lg:max-w-[54%]">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-12">
                ...
                <Button
                  size="lg"
                  variant="outline"
                  className="w-full sm:w-fit border-primary/80 bg-primary/10 text-white hover:bg-primary/20 hover:border-primary hover:text-white hover:scale-105 transform transition-all duration-200 px-8 backdrop-blur-sm shadow-lg hover:shadow-xl"
                  asChild
                >
                  <Link to={config.hero.quoteLink ?? '/get-quote'}>Get a Quote</Link>
                </Button>
      ...
      <section
        ...
        aria-label={config.individuals.ariaLabel ?? config.individuals.title}
      >
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-12">
          <div className="text-center mb-10">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">
              {config.individuals.title}
            </h2>
            <p className="text-base text-gray-500 max-w-xl mx-auto">
              {config.individuals.subtitle}
            </p>
          </div>
                ...
                <div className="absolute inset-0">
                  {currentIndividual.imageKey ? (
                    <ResponsiveImage
                      ...
                      alt={
                        config.individuals.imageAltMap[activeIndividual] ?? currentIndividual.title
                      }
              ...
              <div className="lg:col-span-3 p-7 lg:p-10 flex flex-col justify-center">
                ...
                <div className="flex flex-wrap gap-1.5 mb-7">
                  {currentIndividual.benefits.map((b, i) => (
                    <span
                      key={i}
                      className="inline-flex items-center gap-1 text-[11px] font-medium text-primary bg-primary/8 rounded-full px-2.5 py-1 border border-primary/15"
                    >
                ...
                <div>
                  <Button
                    asChild
                    className="bg-primary text-white hover:bg-primary/90 transition-all duration-200 font-medium text-sm"
                  >
                    <Link to={config.individuals.quoteLink}>
      ...
      {/* ═══ BUSINESS SECTION ═══ */}
      <section
        className="py-20 section-dark-gray"
        aria-label={config.business.ariaLabel ?? config.business.title}
      >
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-12">
          <div className="text-center mb-10">
            ...
            <div className="grid lg:grid-cols-5">
              ...
              <div className="lg:col-span-3 p-7 lg:p-10 flex flex-col justify-center">
                ...
                <p className="text-sm text-gray-300 leading-relaxed mb-6">
                  {currentBusiness.benefitsDescription}
                </p>
                ...
                <div className="flex flex-wrap gap-1.5 mb-7">
                  {currentBusiness.benefits.map((b, i) => (
                    <span
                      key={i}
                      className="inline-flex items-center gap-1 text-[11px] font-medium text-white bg-primary/10 rounded-full px-2.5 py-1 border border-primary/20"
                    >
                      <CheckCircle className="h-3 w-3 flex-shrink-0" /> {b}
                    </span>
                  ))}
                ...
                <div>
                  ...
                  >
                    <Link to={config.business.quoteLink}>
                      Get a Quote <ArrowRight className="ml-2 h-4 w-4" />
                    </Link>
                  </Button>
                </div>
        ...
        <section className="py-20 bg-white" aria-label="Our approach">
          <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-12">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">
                {config.approach.serviceName}
              </h2>
              <p className="text-base text-gray-500 max-w-xl mx-auto">
                {config.approach.headerDescription ??
                  'Our step-by-step approach to delivering exceptional service.'}
              </p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-10">
              {config.approach.steps.map((step, i) => (
                <div
                  key={i}
                  className="relative bg-gray-50 rounded-2xl p-7 border border-gray-100 shadow-sm hover:shadow-md transition-all"
                >
                  <div className="flex items-center gap-4 mb-4">
                    <div
                      className={`w-12 h-12 bg-gradient-to-br ${step.color} rounded-xl flex items-center justify-center shadow-md`}
                    >
            ...
            <div className="text-center">
              <p className="text-sm text-gray-500 max-w-2xl mx-auto mb-6">
                {config.approach.commitmentText ??
                  'We are committed to providing the best possible service to meet your needs.'}
              </p>
              ...
              <div className="lg:col-span-2 section-dark-gray p-9 lg:p-12 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-300 leading-relaxed mb-8">
                    Complete the form and a {config.form.specialistType} specialist will contact you
                    with personalised options tailored to your needs.
                  </p>
                    ...
                    {[
                      {
                        ...
                        sub: 'No cost, no commitment',
                      },
                      { icon: Shield, label: 'Expert advice', sub: 'From accredited specialists' },
                      {
                        icon: TrendingUp,
                        label: 'Market-wide search',
                        sub: 'Best options at the right price',
                      },
                ...
                <div className="mt-10 pt-8 border-t border-white/10">
                  <div className="flex flex-wrap gap-2">
                    {['Free consultation', 'No obligation', 'Quick response'].map((tag) => (
                      <span
                        key={tag}
                        className="inline-flex items-center gap-1.5 text-[11px] font-medium text-gray-300 bg-white/5 rounded-full px-3 py-1.5 border border-white/10"
                      >
                ...
                >
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <Label className="flex items-center gap-2 text-gray-700 text-xs font-semibold uppercase tracking-wide">
                      ...
                      <Input
                        ...
                        className="border-gray-200 focus:border-primary focus:ring-primary text-sm"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="flex items-center gap-2 text-gray-700 text-xs font-semibold uppercase tracking-wide">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      <Label className="flex items-center gap-2 text-gray-700 text-xs font-semibold uppercase tracking-wide">
                        <List className="h-3.5 w-3.5 text-primary" /> {config.form.selectLabel}
                      </Label>
                      <Select
                        value={formData[config.form.selectFieldName] ?? ''}
                        onValueChange={(v) => handleInputChange(config.form.selectFieldName, v)}
                      >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Textarea
                      placeholder={config.form.textareaPlaceholder}
                      rows={4}
                      value={formData.message}
                      onChange={(e) => handleInputChange('message', e.target.value)}
                      className="border-gray-200 focus:border-primary focus:ring-primary resize-none text-sm"
                    />
                  </div>
                  <div className="pt-1">
                    <Button
                      type="submit"
                      size="lg"
                      className="w-full bg-primary text-white hover:bg-primary/90 hover:scale-[1.01] transition-all duration-200 shadow-md font-medium text-sm"
                    >
