...
Hide
Purpose & Scope
Provides guidance on using the Share on LinkedIn feature to enhance content visibility and engagement on the LinkedIn platform.

Value Proposition
Utilizing Share on LinkedIn allows users to reach a professional audience, drive traffic to their sites, and increase content sharing across networks, thereby enhancing their online presence.

Quick Summary of Content
Authentication: New users must authenticate via OAuth 2.0, requesting the w_member_social scope to post on LinkedIn.
Creating Shares: Instructions for sharing text, URLs, and images using the UGC API, including required API requests and headers.
Share Content: Details on structuring the share content, including visibility options and media attachments.
